...
def procesar_bingx(db, uid, ak, as_):
        ...
        if res_s.get("code") == 0 and res_s.get("data"):
            for b in res_s['data']['balances']:
                ...
                if total > 0.000001:
                    ...
                    saldos_count += 1
        print(f"    [OK] BingX Spot: {saldos_count} saldos actualizados.")
    ...
    for item in dict_spot:
        ...
        if t_res.get("code") == 0 and isinstance(t_res.get("data"), list):
            for t in t_res['data']:
                ...
                if registrar_trade_completo(cursor, uid, t_f, item, "BINGX"): total_tr += 1
    actualizar_punto_sincro(cursor, uid, "BINGX", "trades_spot", int(time.time()*1000))
    print(f"    [INFO] BingX Trades: {total_tr} registros procesados.")
    ...
    print(f"    [OK] BingX Open Orders: {total_oo} registradas.")
...
# ==========================================================
def run():
    print(f"💎 MOTOR v6.4.7 - SALDOS + TRADES BINANCE-BINGX")
    while True:
