...
def obtener_punto_inicio_sincro(cursor, broker_id, metodo):
    ...
    row = cursor.fetchone()
    return int(row['ultima_fecha_sincro']) if row and row['ultima_fecha_sincro'] else int((time.time() - (30 * 24 * 60 * 60)) * 1000)
...
def actualizar_punto_sincro(cursor, broker_id, metodo, nuevo_ts):
    sql = """
        ...
        VALUES (%s, %s, %s, NOW())
        ON DUPLICATE KEY UPDATE ultima_fecha_sincro = VALUES(ultima_fecha_sincro), last_update = NOW()
    """
...
# ==========================================================
def run():
    print("💎 MOTOR SALDOS v6.00.1 - FASE 1 ACTIVA Y GARANTIZADA")
    while True:
        db = None
        try:
            ...
            for u in users:
                # Bloqueo por usuario y broker específico
                if not obtener_permiso_ejecucion(cursor, u['user_id'], f"motor_saldos_{u['broker_name'].lower()}"):
                    continue
                ...
                # Preparación para delta de tiempo (Fase 1 completada)
                start_ts = obtener_punto_inicio_sincro(cursor, u['broker_id'], "saldo_sync")
