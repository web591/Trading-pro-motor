...
def procesar_bingx(db, uid, ak, as_):
    ...
    # 1. CARGA DE TRADUCTORES (Cargamos TODO lo de BingX de cualquier categoría)
    # Esto permite que si una orden es FOREX pero está en la API de Futures, la encuentre.
    cursor.execute("""
        SELECT id, ticker_motor, underlying, motor_fuente 
        FROM sys_traductor_simbolos 
        WHERE motor_fuente LIKE 'bingx_%' AND is_active = 1
    """)
    traductores_bingx = cursor.fetchall()

    def buscar_en_traductor(simbolo_api):
        """
        Lógica de rescate:
        1. Busca match exacto (BTC-USDT == BTC-USDT)
        2. Busca match parcial (Si AAPLX-USDT contiene AAPL de tu catálogo)
        3. Busca por Underlying (Si el underlying del traductor está en el símbolo de la API)
        """
        # Limpieza básica
        simbolo_clean = simbolo_api.replace("-USDT", "").replace("USDT", "")
        
        # Intento 1: Match Exacto
        for t in traductores_bingx:
            if t['ticker_motor'] == simbolo_api:
                return t
        
        # Intento 2: Match por Underlying (El "AAPL" del traductor está en "AAPLX-USDT")
        for t in traductores_bingx:
            if t['underlying'] and t['underlying'] in simbolo_api:
                return t
                
        return None

    # 2. PROCESAR SALDOS (Simplificado para brevedad, usa la misma lógica de búsqueda)
    # ... (Mantener lógica de saldos v6.5.0)
    ...
    for o in orders_s:
        symbol = o.get('symbol')
        info = buscar_en_traductor(symbol)
        if info:
    ...
    # 4. OPEN ORDERS FUTURES (Aquí es donde entraban los AAPLX, XAUT, etc.)
    ...
    for o in orders_f:
        ...
        if info:
            ...
            print(f"    [OK] Orden Futuros registrada: {symbol} (Asociada a ID:{info['id']} - {info['underlying']})")
        else:
            print(f"    [ADVERTENCIA] No se pudo mapear '{symbol}'. Ni por ticker exacto ni por underlying.")
...
# ==========================================================
def run():
    print(f"💎 MOTOR v6.5.1 - SALDOS + TRADES BINANCE-BINGX")
    while True:
