...
def procesar_bingx(db, uid, ak, as_):
    ...
    try:
        ...
        if res_spot.get("data"):
            for b in res_spot['data']['balances']:
                ...
                info = obtener_datos_traductor(cursor, "bingx_crypto", b['asset'])
    ...
    try:
        ...
        for item in balances:
            ...
            info = obtener_datos_traductor(cursor, "bingx_usdt_future", ticker)
            tid = info['id'] if info else None
            registrar_saldo(cursor, uid, tid, float(item['balance']), float(item.get('freezedMargin', 0)), ticker, "BINGX", "FUTURES")
        ...
        for item in diccionario:
            ...
            try:
                ...
                for t in (raw_trades or []):
                    ...
                    registrar_trade_completo(cursor, uid, t_f, item)
            except: continue
...
def run():
    print(f"💎 MOTOR v6.3.2 - START: 01-ENE-2026")
    while True:
