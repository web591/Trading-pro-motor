...
def registrar_trade_completo(cursor, uid, t_data):
    try:
        ...
        # Basado en tu SQL: (user_id, monto_fiat, monto_total, moneda, tipo_movimiento, id_externo, fecha_transaccion)
        sql_global = """
            INSERT IGNORE INTO transacciones_globales 
            (user_id, monto_fiat, monto_total, moneda, tipo_movimiento, id_externo, fecha_transaccion) 
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """
        # quoteQty es el valor en USDT (monto fiat/total en este contexto)
        monto = t_data['quoteQty'] 
        tipo = 'COMPRA' if t_data['side'].upper() == 'BUY' else 'VENTA'
        
        cursor.execute(sql_global, (
            uid, monto, monto, t_data['symbol'], tipo, t_data['orderId'], t_data['fecha_sql']
        ))
        
        # 2. detalle_trades
        sql_detalle = """
            INSERT IGNORE INTO detalle_trades 
            (id_externo_ref, symbol, lado, precio_ejecucion, cantidad_ejecutada, trade_id_externo, user_id, fecha_utc) 
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
        """
        cursor.execute(sql_detalle, (
            t_data['orderId'], t_data['symbol'], t_data['side'], 
            t_data['price'], t_data['qty'], f"API-{t_data['orderId']}", uid, t_data['fecha_sql']
        ))
        
        print(f"        [✓] GUARDADO: {t_data['symbol']} | {monto} USDT")
        return True
