...
def procesar_binance_um_futures(db, uid, k, s):
    ...
    try:
        ...
        for b in acc:
            ...
            if total > 0.000001:
                info = obtener_traductor_id(cursor, "binance_um_futures", b['asset'])
        ...
        cursor.execute("""
            ...
            FROM sys_traductor_simbolos
            WHERE motor_fuente = 'binance_um_futures'
        """)
        ...
        # ======================================================
        # 3️⃣ OPEN ORDERS FUTURES (UM) - FIX POR SYMBOL
        # ======================================================
        try:
            ...
            for row in simbolos:
                ...
                try:
                    ...
                    for oo in open_orders:
                        ...
                        info = obtener_traductor_id(cursor, "binance_um_futures", symbol)
                        ...
                        cursor.execute(sql, (
                            ...
                            0.0,
                            datetime.fromtimestamp(oo['time']/1000).strftime('%Y-%m-%d %H:%M:%S'),
                            'ABIERTA'
                        ))
                    ...
                    print(f"        [UM OPEN WARN] {symbol}: {e}")
                    continue

            print(f"    [UM] Open Orders: {oo_count}")

        except Exception as e:
            print(f"    [UM OPEN ERROR] {e}")
...
def procesar_binance_cm_futures(db, uid, k, s):
    ...
    try:
        ...
        # ======================================================
        # 2️⃣ OPEN ORDERS CM (FIX POR SYMBOL)
        # ======================================================
        cursor.execute("""
            SELECT ticker_motor
            FROM sys_traductor_simbolos
...
# ==========================================================
def run():
    print(f"💎 MOTOR v6.6.6.12 - SALDOS + TRADES BINANCE-BINGX")
    while True:
