...
def procesar_bingx(db, uid, ak, as_):
    ...
    # 2. SALDOS SPOT (Basado en debug: data.get("data", {}).get("balances", []))
    try:
        ...
        for b in balances:
            ...
            if total > 0:
                asset = b.get('asset')
                info = next((i for i in dict_spot if i['ticker_motor'].replace('-', '') == asset), None)
                registrar_saldo(cursor, uid, info, total, float(b.get('locked', 0)), asset, "BINGX", "SPOT")
                s_count += 1
        print(f"    [OK] BingX Spot: {s_count} activos actualizados.")
    except Exception as e: print(f"    [!] Error BingX Spot Balance: {e}")

    # 3. SALDOS FUTURES PERPETUAL (Basado en debug: data.get("data", {}).get("balance"))
    try:
        ...
        if isinstance(bal_data, dict):
            ...
            # Usamos freezedMargin si existe, o calculamos diferencia
            locked = float(bal_data.get("freezedMargin", 0)) 
            info = next((i for i in dict_fut if i['ticker_motor'].replace('-', '') == asset), None)
            registrar_saldo(cursor, uid, info, total, locked, asset, "BINGX", "FUTURES")
            print(f"    [OK] BingX Futures Perpetual actualizado ({asset}).")
    except Exception as e: print(f"    [!] Error BingX Futures Balance: {e}")

    # 4. OPEN ORDERS SPOT (Basado en debug: data.get("data", {}).get("orders", []))
    ...
    for o in orders_s:
        oo_spot += 1
        info = next((i for i in dict_spot if i['ticker_motor'] == o.get('symbol')), None)
        ...
        cursor.execute(sql, (str(o.get('orderId')), uid, "BINGX", info['id'] if info else None, o.get('symbol'), o.get('side'), o.get('type'), float(o.get('price', 0)), float(o.get('origQty', 0)), float(o.get('origQuoteOrderQty', 0)), datetime.fromtimestamp(o.get('time', 0)/1000).strftime('%Y-%m-%d %H:%M:%S'), 'ABIERTA'))
    print(f"    [OK] BingX Open Orders Spot: {oo_spot} registradas.")

    # 5. OPEN ORDERS FUTURES (Basado en debug: data.get("data", {}).get("orders", []))
    ...
    for o in orders_f:
        oo_fut += 1
        info = next((i for i in dict_fut if i['ticker_motor'] == o.get('symbol')), None)
        ...
        cursor.execute(sql, (str(o.get('orderId')), uid, "BINGX", info['id'] if info else None, o.get('symbol'), o.get('side'), o.get('type', 'LIMIT'), float(o.get('price', 0)), float(o.get('origQty', 0)), 0.0, datetime.fromtimestamp(o.get('updateTime', 0)/1000).strftime('%Y-%m-%d %H:%M:%S'), 'ABIERTA'))
    print(f"    [OK] BingX Open Orders Futures: {oo_fut} registradas.")

    # 6. TRADES SPOT (Forzado Enero 2026)
    t_count = 0
    start_ts = 1735689600000 
    for item in dict_spot:
        res_t = bx_req("/openApi/spot/v1/trade/myTrades", {"symbol": item['ticker_motor'], "startTime": start_ts})
        trades = res_t.get("data", [])
    ...
    print(f"    [INFO] BingX Spot Trades: {t_count} procesados.")
...
# ==========================================================
def run():
    print(f"💎 MOTOR v6.4.8 - SALDOS + TRADES BINANCE-BINGX")
    while True:
