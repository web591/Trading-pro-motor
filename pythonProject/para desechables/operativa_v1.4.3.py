...
def registrar_orden_abierta(cursor, uid, exchange_str, symbol, order_id, side, price, qty, type_order, ts, mercado='SPOT'):
    ...
    # Insertamos con los campos mínimos que toda tabla de órdenes tiene
    # Si falla por 'exchange' o 'broker_id', el try/except lo maneja
    try:
        cursor.execute("""
            INSERT INTO sys_open_orders
            (user_id, symbol, order_id, side, price, qty, type, traductor_id, fecha_utc)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,FROM_UNIXTIME(%s/1000))
            ON DUPLICATE KEY UPDATE
            price=VALUES(price), qty=VALUES(qty), type=VALUES(type), fecha_utc=VALUES(fecha_utc)
        """, (uid, symbol, order_id, side, price, qty, type_order, tid, ts))
    except Exception as e:
        print(f"⚠️ Error al registrar orden abierta: {e}")
...
def run():
    print("🚀 MOTOR INSTITUCIONAL v1.4.3 - FULL ECOSISTEMA")
    while True:
        try:
