...
def binance_dividends(db, user_id, key, secret):
    ...
    # Obtenemos el último sync
    last_sync = obtener_sync(cursor, user_id, "BINANCE", endpoint)
    
    # FORZAR HISTORIAL: Si es la primera vez (0), ir 90 días atrás. 
    # Si no, ir desde el último registro.
    if last_sync == 0:
        start_time = int((time.time() - (90 * 24 * 3600)) * 1000)
    else:
        start_time = last_sync + 1

    print(f"    [+] {endpoint}: Buscando desde {time.strftime('%Y-%m-%d', time.gmtime(start_time/1000))}...")
    ...
    if "rows" in r:
        ...
        for d in r["rows"]:
            ...
            # EL CAMPO EN LA API ES 'id', NO 'tranId'
            id_ext = f"BN-DIV-{d['id']}" 
            ...
            registrar_cashflow(cursor, {
                ...
                "ticker_motor": d.get("enInfo", "Earn/Staking"), # Información del beneficio
...
def ejecutar_motor_financiero(db):
    print(f"\n{'='*60}")
    print(f"💎 MOTOR FINANCIERO v1.2.1 - AUDITORÍA Y DIVIDENDOS")
    print(f"{'='*60}")
