...
def ejecutar_motor_financiero(db):
    ...
    for u in usuarios:
        ...
        print("Procesando:",u["broker_name"],u["user_id"])

        key=descifrar_dato(u["api_key"],MASTER_KEY)
        secret=descifrar_dato(u["api_secret"],MASTER_KEY)
        ...
        if u["broker_name"].upper()=="BINANCE":
            ...
            binance_income(db,u["user_id"],key,secret)

            binance_deposits(db,u["user_id"],key,secret)

            binance_withdraw(db,u["user_id"],key,secret)

            binance_convert_history(db,u["user_id"],key,secret)

            binance_dust_log(db,u["user_id"],key,secret)

            binance_staking_rewards(db,u["user_id"],key,secret)

        if u["broker_name"].upper()=="BINGX":

            bingx_income(db,u["user_id"],key,secret)
