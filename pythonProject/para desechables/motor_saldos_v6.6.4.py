...
def procesar_bingx(db, uid, ak, as_):
    ...
    balances_spot = res_s.get('data', {}).get('balances', [])
    for b in balances_spot:
    ...
    # B. Saldo FUTUROS (Perpetual Swap) - Aquí suele estar el USDT de margen
    res_f = bx_req("/openApi/swap/v2/user/balance")
    balances_fut = res_f.get('data', [])
    if isinstance(balances_fut, list):
        for bf in balances_fut:
            # En BingX Futuros el campo es 'balance' y 'asset'
            total = float(bf.get('balance', 0))
            equity = float(bf.get('equity', 0)) # Valor con PNL no realizado si lo necesitas
            if total > 0.000001:
                asset = bf.get('asset')
                info = buscar_en_traductor_bingx(asset)
                # En futuros el 'locked' es el margen usado (frozen)
                locked = total - float(bf.get('availableMargin', total))
                registrar_saldo(cursor, uid, info, total, locked, asset, "BINGX", "FUTURES")
                s_count += 1
                
    print(f"    [OK] BingX Saldos actualizado: {s_count} activos totales.")
    ...
    # --- 4. SINCRONIZACIÓN DE TRADES (HISTORIAL) ---
    # Usamos el punto de inicio guardado para no duplicar
    ...
    if isinstance(trades_raw, list):
        for t in trades_raw:
            # Solo procesamos órdenes llenas o parcialmente llenas
            if t.get('status') in ['FILLED', 'PARTIALLY_FILLED']:
                sym = t.get('symbol')
                info = buscar_en_traductor_bingx(sym)
                
                # Formateamos para que registrar_trade_completo lo entienda
                t_f = {
                    ...
                    'commission': float(t.get('fee', 0)),
                    'commissionAsset': 'USDT', # BingX Futures suele cobrar en USDT
                    'fecha_sql': datetime.fromtimestamp(t['updateTime']/1000).strftime('%Y-%m-%d %H:%M:%S'),
                    'isMaker': False # La API simplificada no siempre lo da
                }
                
                # Registramos (la función ya maneja el INSERT IGNORE)
                if registrar_trade_completo(cursor, uid, t_f, info, "BINGX"):
                    t_count += 1
        
        # Actualizamos el timestamp para la próxima vuelta
        if trades_raw:
            # Usamos el tiempo actual para la siguiente ventana
...
# ==========================================================
def run():
    print(f"💎 MOTOR v6.6.4 - SALDOS + TRADES BINANCE-BINGX")
    while True:
