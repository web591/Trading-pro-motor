...
# ==========================================================
# 🕒 GESTIÓN DE TIEMPO Y SINCRONIZACIÓN (VALIDADO)
# ==========================================================
def normalizar_fecha_sql(ts_ms):
    try:
        ts_segundos = float(ts_ms) / 1000.0
...
def obtener_punto_inicio_sincro(cursor, llave_id, metodo):
    """
    CONSULTA START_TIME: Corregido para usar la columna 'id' de la tabla api_keys.
    """
    # Usamos id_llave_api para que coincida con tu estructura de sys_sync_estado
...
def actualizar_punto_sincro(cursor, llave_id, metodo, nuevo_ts):
    """
    ACTUALIZA EL PUNTERO: Usa el ID de la llave para saber qué cuenta se sincronizó.
    """
    sql = """
        ...
        last_update = NOW()
    """
    cursor.execute(sql, (llave_id, metodo, nuevo_ts))

# ==========================================================
# 🛡️ SEGURIDAD DE PROCESOS (VALIDADO)
# ==========================================================
def obtener_permiso_ejecucion(cursor, uid, proceso):
    try:
...
# ... (Tus funciones de seguridad y traductor se mantienen igual)

# ==========================================================
# 🚀 EJECUCIÓN MAESTRA (VALIDADO v6.0.1)
# ==========================================================
def run():
    print("💎 MOTOR SALDOS v6.0.2 - VALIDACIÓN SQL TOTAL")
    while True:
        db = None
        try:
            ...
            # SELECCIÓN CORREGIDA: Solo campos existentes en tu tabla api_keys
            cursor.execute("SELECT id, user_id, api_key, api_secret, broker_name FROM api_keys WHERE status=1")
            users = cursor.fetchall()

            for u in users:
                # 1. SEGURIDAD: Bloqueo para evitar que se pisen los procesos
                nombre_proceso = f"motor_saldos_{u['broker_name'].lower()}"
                if not obtener_permiso_ejecucion(cursor, u['user_id'], nombre_proceso):
                    print(f"    [!] Proceso {nombre_proceso} para usuario {u['user_id']} ya en curso. Saltando...")
                    continue
                ...
                # 3. SINCRONIZACIÓN: Usamos el 'id' de la tabla api_keys
                start_ts = obtener_punto_inicio_sincro(cursor, u['id'], "saldo_sync")
                ...
                # Guardamos los saldos actualizados
                db.commit()
