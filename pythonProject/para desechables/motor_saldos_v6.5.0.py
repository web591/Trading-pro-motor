...
# ==========================================================
# 🟦 PROCESADOR BINGX (CON LOGS ESTRATÉGICOS)
# ==========================================================
def procesar_bingx(db, uid, ak, as_):
    ...
    # 1. CARGA DE TRADUCTORES (Cargamos TODO lo de BingX para mayor cobertura)
    cursor.execute("SELECT id, ticker_motor, motor_fuente FROM sys_traductor_simbolos WHERE motor_fuente LIKE 'bingx_%' AND is_active = 1")
    traductores_bingx = cursor.fetchall()
    
    # Separamos para facilitar la búsqueda rápida
    dict_spot = [i for i in traductores_bingx if i['motor_fuente'] == 'bingx_crypto']
    dict_fut = [i for i in traductores_bingx if i['motor_fuente'] == 'bingx_usdt_future']

    # 2. SALDOS SPOT
    try:
        res_s = bx_req("/openApi/spot/v1/account/balance")
        balances = res_s.get("data", {}).get("balances", [])
        for b in balances:
            total = float(b.get('free', 0)) + float(b.get('locked', 0))
            if total > 0:
                asset = b.get('asset')
                # Buscamos coincidencia flexible (BTC-USDT o BTC)
                info = next((i for i in dict_spot if i['ticker_motor'].replace('-', '') == asset or i['ticker_motor'].split('-')[0] == asset), None)
                registrar_saldo(cursor, uid, info, total, float(b.get('locked', 0)), asset, "BINGX", "SPOT")
    except Exception as e: print(f"    [!] Error en Saldos Spot: {e}")

    # 3. SALDOS FUTURES
    try:
        res_f = bx_req("/openApi/swap/v2/user/balance")
        bal_data = res_f.get("data", {}).get("balance")
        if isinstance(bal_data, dict):
            asset = bal_data.get("asset")
            total = float(bal_data.get("balance", 0))
            locked = float(bal_data.get("freezedMargin", 0)) 
            info = next((i for i in dict_fut if asset in i['ticker_motor']), None)
            registrar_saldo(cursor, uid, info, total, locked, asset, "BINGX", "FUTURES")
    except Exception as e: print(f"    [!] Error en Saldos Futures: {e}")
    ...
    for o in orders_s:
        symbol = o.get('symbol')
        # Buscamos el ID en el traductor
        info = next((i for i in dict_spot if i['ticker_motor'] == symbol), None)
        ...
        if info:
            ...
            cursor.execute(sql, (str(o.get('orderId')), uid, "BINGX", info['id'], symbol, o.get('side'), o.get('type'), float(o.get('price', 0)), float(o.get('origQty', 0)), float(o.get('origQuoteOrderQty', 0)), datetime.fromtimestamp(o.get('time', 0)/1000).strftime('%Y-%m-%d %H:%M:%S'), 'ABIERTA'))
        else:
            print(f"    [ADVERTENCIA] Orden Spot ignorada: Símbolo {symbol} no está en sys_traductor_simbolos para BingX.")

    # 5. OPEN ORDERS FUTURES
    ...
    for o in orders_f:
        symbol = o.get('symbol')
        # Aquí es donde fallaba: si no hay bingx_usdt_future, buscamos en cualquier bingx_
        info = next((i for i in traductores_bingx if i['ticker_motor'] == symbol), None)
        ...
        if info:
            ...
            cursor.execute(sql, (str(o.get('orderId')), uid, "BINGX", info['id'], symbol, o.get('side'), o.get('type', 'LIMIT'), float(o.get('price', 0)), float(o.get('origQty', 0)), 0.0, datetime.fromtimestamp(o.get('updateTime', 0)/1000).strftime('%Y-%m-%d %H:%M:%S'), 'ABIERTA'))
        else:
            print(f"    [ADVERTENCIA] Orden Futuros ignorada: Símbolo {symbol} no mapeado en traductor (BingX).")

    print(f"    [OK] Ciclo BingX finalizado para User {uid}")
...
# ==========================================================
def run():
    print(f"💎 MOTOR v6.5.0 - SALDOS + TRADES BINANCE-BINGX")
    while True:
