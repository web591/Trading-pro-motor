...
def registrar_trade_completo(cursor, uid, t_data):
    try:
        ...
        """
        valores_global = (id_vinculo, uid, 'BINANCE', 'SPOT', 'TRADE', t_data['symbol'], t_data['quoteQty'], t_data['commission'], t_data['fecha_sql'], 'BINANCE')
        cursor.execute(sql_global, valores_global)

        sql_detalle = """
            INSERT IGNORE INTO detalle_trades 
            (user_id, exchange, tipo_producto, exchange_fuente, tipo_mercado, id_externo_ref, fecha_utc, symbol, lado, precio_ejecucion, cantidad_ejecutada, broker, trade_id_externo) 
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        """
        valores_detalle = (uid, 'BINANCE', 'SPOT', 'BINANCE', 'SPOT', id_vinculo, t_data['fecha_sql'], t_data['symbol'], t_data['side'], t_data['price'], t_data['qty'], 'BINANCE', f"TRD-{id_vinculo}")
...
# ==========================================================
def procesar_bingx(db, uid, api_key, api_secret):
    try:
        ...
        url = f"{base_url}{uri}?{query_string}&signature={signature}"
        ...
        if data.get('code') == 0:
            ...
            for b in balances:
                ...
                if total > 0.000001:
                    asset = b['asset']
                    tid = obtener_traductor_id(cursor, "bingx_spot", asset)
                    ...
                    sql = """
                        ...
                        VALUES (%s, 'BINGX', %s, %s, %s, %s, %s, %s, %s, 'SPOT', NOW()) 
                        ON DUPLICATE KEY UPDATE cantidad_total=VALUES(cantidad_total), valor_usd=VALUES(valor_usd), last_update=NOW()
                    """
                    cursor.execute(sql, (uid, asset, tid, total, free, locked, total * precio, precio))
            print(f"    [OK] BingX Saldos actualizado.")
    except Exception as e:
        print(f"    [!] Error BingX User {uid}: {e}")
...
def procesar_binance(db, uid, k, s):
    try:
        ...
        # HISTORIAL (Lógica 6.2.8 estable)
        ...
        print(f"    [INFO] Binance Trades: {total_ingresados} nuevos.")
...
# ==========================================================
def run():
    print(f"💎 MOTOR v6.2.9 - BINANCE & BINGX ESTABLE")
    while True:
