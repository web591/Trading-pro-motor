...
def binance_dividends(db, user_id, key, secret):
    ...
    # 1. Recuperamos el último timestamp guardado
    ...
    # 2. Si ya tenemos historial, solo pedimos lo NUEVO (last_sync + 1)
    if last_sync > 0:
        params["startTime"] = last_sync + 1
        print(f"    [+] {endpoint}: Buscando nuevos desde {time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(last_sync/1000))}...")
    else:
        print(f"    [+] {endpoint}: Sincronizando historial inicial (últimos 500)...")
    ...
    try:
        ...
        if "rows" in r and r["rows"]:
            ...
            # 3. Solo actualizamos el sync si realmente encontramos algo más nuevo
            if max_ts > last_sync:
                guardar_sync(cursor, user_id, "BINANCE", endpoint, max_ts)
        
        print(f"    [OK] {endpoint}: {count} procesados.")
        
    except Exception as e:
        print(f"    [!] Error en BINANCE_DIVIDEND: {e}")
...
def binance_dust_log(db, uid, key, secret):
    ...
    # Vamos a ignorar el last_sync un momento para limpiar el error
    # Solo mandaremos el timestamp para ver si responde
    timestamp = int(time.time() * 1000)
    ...
    # Generamos la query y la firma de forma manual y estricta
    ...
    print(f"    [+] {endpoint}: Verificando conexión con API...")
    
    try:
        r = requests.get(url, headers={"X-MBX-APIKEY": key}).json()
        
        # Si sigue dando error de 'endTTime', es que Binance detecta algo mal en los headers o la firma
        if "userAssetDribblets" not in r:
            print(f"    [!] {endpoint}: Error persistente: {r}")
            return

        dribblets = r["userAssetDribblets"]
        count = 0
        max_ts = 0
        
        for entry in dribblets:
        ...
        if count > 0:
            guardar_sync(cursor, uid, "BINANCE", endpoint, max_ts)
            print(f"    [OK] {endpoint}: {count} conversiones recuperadas.")
        else:
            print(f"    [OK] {endpoint}: No hay datos recientes.")

    except Exception as e:
        print(f"    [!] Error en DUST: {e}")
...
def binance_convert_history(db, uid, key, secret):
    ...
    # Inicio: Octubre 2021 o el último sync
    ...
    while start_ts < end_now:
        # Binance solo permite rangos de 30 días por petición en Convert
        chunk_end = start_ts + (30 * 24 * 60 * 60 * 1000)
        if chunk_end > end_now: chunk_end = end_now

        print(f"        [..] Convert: Escaneando bloque {time.strftime('%Y-%m-%d', time.gmtime(start_ts/1000))}...", end="\r")
        ...
        try:
            ...
            rate_limit() # Evitar baneo de IP
            
        except Exception as e:
            print(f"    [!] Error en bloque Convert: {e}")
            break

    print(f"    [OK] {endpoint}: Historial actualizado.")
...
def ejecutar_motor_financiero(db):
    print(f"\n{'='*60}")
    print(f"💎 MOTOR FINANCIERO v1.2.5 - AUDITORÍA Y DIVIDENDOS")
    print(f"{'='*60}")
