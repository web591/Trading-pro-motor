...
def registrar_orden_abierta(cursor, uid, exchange_str, symbol, order_id, side, price, qty, type_order, ts, mercado='SPOT'):
    """
    Mercado puede ser 'SPOT' o 'FUTURES' según tus tablas separadas
    """
    tid = obtener_traductor_id(cursor, exchange_str.lower(), symbol)
    
    # Si tienes tablas separadas sys_open_orders_spot / sys_open_orders_futures:
    tabla = "sys_open_orders" if mercado == 'SPOT' else "sys_open_orders_futures"
    
    # Validamos si la tabla existe en tu SQL (sys_open_orders es la estándar en tu dump)
    # Si usas una sola, agregamos el campo mercado si existe
    cursor.execute(f"""
        INSERT INTO {tabla}
        (user_id, exchange, symbol, order_id, side, price, qty, type, traductor_id, fecha_utc)
        VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,FROM_UNIXTIME(%s/1000))
        ON DUPLICATE KEY UPDATE
        price=VALUES(price), qty=VALUES(qty), type=VALUES(type), fecha_utc=VALUES(fecha_utc)
    """, (uid, exchange_str, symbol, order_id, side, price, qty, type_order, tid, ts))
...
def procesar_binance(db, uid, api_key, api_secret, b_id):
    ...
    cursor.execute("DELETE FROM sys_open_orders WHERE user_id=%s AND exchange='BINANCE'", (uid,))
    # cursor.execute("DELETE FROM sys_open_orders_futures WHERE user_id=%s AND exchange='BINANCE'", (uid,))
            ...
            if trades:
                for t in trades:
                    ...
                    if float(t['commission']) > 0:
                        insertar_tx(cursor, uid, "BINANCE", "FEE", -float(t['commission']), t['commissionAsset'], t['time'], "SPOT_FEE", f"FEE_{t['id']}")
...
def ejecutar_conciliacion(db):
    ...
    for r in registros:
        ...
        cursor.execute("""
            ...
            (user_id, exchange, asset, ledger_total, snapshot_total, diferencia, estado, fecha_revision)
            VALUES (%s,%s,%s,%s,%s,%s,%s,NOW())
        """, (r['user_id'], r['exchange'], r['asset'], ledger, snapshot, diff, "OK" if abs(diff) < 0.00001 else "DESCUADRE"))
    db.commit()

def run():
    print("🚀 MOTOR INSTITUCIONAL v1.4 - FULL ECOSISTEMA")
    while True:
        try:
