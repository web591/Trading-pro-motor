...
def procesar_bingx(db, uid, ak, as_):
    ...
    def buscar_en_traductor_bingx(simbolo_api):
        """
        LÓGICA v6.6.8: Cruza lo que manda la API contra la DB 
        usando limpieza total de guiones para garantizar el match.
        """
        ...
        # BUSQUEDA NIVEL 1: Match directo o por Ticker Limpio o por Underlying
        ...
        cursor.execute(sql, (s_api, s_limpio, und_api))
        res = cursor.fetchone()
    ...
    # --- 3. OPEN ORDERS FUTURES (BTC-USDT, XAUT-USDT, etc.) ---
    ...
    for o in orders_f:
        ...
        if info:
            ...
            cursor.execute(sql, (str(o['orderId']), uid, "BINGX", info['id'], sym, o['side'], 'LIMIT', float(o['price']), float(o['origQty']), 0.0, datetime.fromtimestamp(o.get('updateTime', o.get('time', 0))/1000).strftime('%Y-%m-%d %H:%M:%S'), 'ABIERTA'))
            print(f"    [OK] Orden Futuros registrada: {sym} (Asociada a ID:{info['id']})")
        else:
            # Si llega aquí, es que BTC-USDT no está en sys_traductor_simbolos con motor_fuente 'bingx_%'
            print(f"    [ADVERTENCIA] No se pudo mapear '{sym}'. Verifique sys_traductor_simbolos.")
...
# ==========================================================
def run():
    print(f"💎 MOTOR v6.6.1 - SALDOS + TRADES BINANCE-BINGX")
    while True:
