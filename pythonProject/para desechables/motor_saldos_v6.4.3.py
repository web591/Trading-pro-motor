        ...
        try:
            ...
            if res_os.get("code") == 0 and isinstance(res_os.get("data"), list):
                for o in res_os['data']:
                    ...
                    cursor.execute(sql, (
                        str(o['orderId']), uid, "BINGX", item['id'], o['symbol'], o['side'], o['type'], float(o['price']), float(o['origQty']), float(o['origQuoteQty']), datetime.fromtimestamp(o['time']/1000).strftime('%Y-%m-%d %H:%M:%S'),'ABIERTA'))
        except: continue
        ...
        try:
            ...
            if res_of.get("code") == 0 and isinstance(res_of.get("data"), list):
                for o in res_of['data']:
                    ...
                    cursor.execute(sql, (
                        str(o['orderId']), uid, "BINGX", item['id'], o['symbol'], 
                        o['side'], o['type'], float(o['price']), float(o['origQty']), 
                        0.0, # Futuros no suele bloquear balance quote de la misma forma que spot
                        datetime.fromtimestamp(o['updateTime']/1000).strftime('%Y-%m-%d %H:%M:%S'), 
                        'ABIERTA'
                    ))
        except: continue
...
# --- BINANCE OPEN ORDERS ---
    ...
    try:
        # Obtenemos todas las órdenes abiertas de una sola vez (Binance lo permite)
        open_orders = client.get_open_orders()
        for oo in open_orders:
            info = obtener_traductor_id(cursor, "binance_spot", oo['symbol'])
            if info:
                sql_oo = """INSERT INTO sys_open_orders_spot 
                            (id_order_ext, user_id, broker_name, traductor_id, symbol, side, type, price, qty, locked_amount, fecha_utc, estado, last_seen) 
                            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,NOW())"""
                cursor.execute(sql_oo, (
                    str(oo['orderId']), uid, "BINANCE", info['id'], oo['symbol'], 
                    oo['side'], oo['type'], float(oo['price']), float(oo['origQty']), 
                    0.0, datetime.fromtimestamp(oo['time']/1000).strftime('%Y-%m-%d %H:%M:%S'), 'ABIERTA'
                ))
        print(f"    [OK] Binance Open Orders: {len(open_orders)} registradas.")
    except Exception as e: print(f"    [!] Error Binance Open Orders: {e}")

# ==========================================================
# 🚀 EJECUCIÓN PRINCIPAL
# ==========================================================
def run():
    print(f"💎 MOTOR v6.4.3 - SALDOS + TRADES BINANCE-BINGX")
    while True:
