...
# ==========================================================
# 🟦 PROCESADOR BINGX (V6.6.5 - FIXED MAPPING & LOGS)
# ==========================================================
def procesar_bingx(db, uid, ak, as_):
    ...
    def buscar_en_traductor_bingx(simbolo_api):
        """
        LÓGICA MAESTRA v6.6.5: 
        Match por ticker exacto, ticker sin guiones o underlying.
        """
        if not simbolo_api: return None
        tk_api_raw = simbolo_api.upper()
        tk_api_limpio = tk_api_raw.replace("-", "").replace("/", "").replace("=X", "")
        und_api = tk_api_limpio.replace("USDT", "").replace("USDC", "")

        # Buscamos en todas las variantes posibles de BingX
        ...
        cursor.execute(sql, (tk_api_raw, tk_api_limpio, und_api))
        res = cursor.fetchone()
        
        # Rescate para CFDs (AAPL, TSLA...)
        if not res:
            sql_cfd = """SELECT id, ticker_motor, underlying, categoria_producto, tipo_investment 
                         ...
                         AND is_active = 1 LIMIT 1"""
            cursor.execute(sql_cfd, (tk_api_limpio,))
            res = cursor.fetchone()
        return res
    ...
    for b in balances:
        ...
        if total > 0.000001:
            ...
            s_count += 1
    print(f"    [OK] BingX Saldos actualizado: {s_count} activos.")
    ...
    # --- 3. OPEN ORDERS FUTURES (Aquí entran BTC-USDT, XAUT-USDT, AAPLX...) ---
    ...
    for o in res_of.get("data", {}).get("orders", []):
        ...
        if info:
            ...
            cursor.execute(sql, (str(o['orderId']), uid, "BINGX", info['id'], sym, o['side'], 'LIMIT', float(o['price']), float(o['origQty']), 0.0, datetime.fromtimestamp(o['updateTime']/1000).strftime('%Y-%m-%d %H:%M:%S'), 'ABIERTA'))
            print(f"    [OK] Orden Futuros registrada: {sym} (Asociada a ID:{info['id']})")
        else:
            print(f"    [ADVERTENCIA] No se pudo mapear '{sym}'.")
...
# ==========================================================
def run():
    print(f"💎 MOTOR v6.6.0 - SALDOS + TRADES BINANCE-BINGX")
    while True:
