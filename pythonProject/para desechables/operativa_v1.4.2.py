...
def registrar_orden_abierta(cursor, uid, exchange_str, symbol, order_id, side, price, qty, type_order, ts, mercado='SPOT'):
    # Convertimos el nombre (BINANCE) al ID numérico (1) para que coincida con tu tabla
    b_id = BROKER_MAP.get(exchange_str.upper(), 0)
    tid = obtener_traductor_id(cursor, exchange_str.lower(), symbol)
    
    # Ajustado a las columnas reales de tu SQL: broker_id en lugar de exchange
    cursor.execute("""
        INSERT INTO sys_open_orders
        (user_id, broker_id, symbol, order_id, side, price, qty, type, traductor_id, fecha_utc)
    ...
    """, (uid, b_id, symbol, order_id, side, price, qty, type_order, tid, ts))
...
def procesar_binance(db, uid, api_key, api_secret, b_id):
    ...
    # Usamos b_id (que es 1 para Binance) en lugar de la palabra 'BINANCE'
    cursor.execute("DELETE FROM sys_open_orders WHERE user_id=%s AND broker_id=%s", (uid, b_id))
