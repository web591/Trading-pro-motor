...
def procesar_binance(db, uid, k, s):
    try:
        ...
        for b in acc['balances']:
            ...
            if total > 0.000001:
                activos_con_saldo.append(b['asset'])
                tid = obtener_traductor_id(cursor, "binance_spot", b['asset'])
                registrar_saldo(cursor, uid, tid, total, float(b['locked']), b['asset'], "BINANCE")
        print(f"    [Saldos] {len(activos_con_saldo)} activos actualizados.")

        # 2. TRADES (Estrategia de validación)
        start_ts = obtener_punto_inicio_sincro(cursor, uid, "BINANCE", "trades_spot")
        fecha_legible = datetime.fromtimestamp(start_ts/1000).strftime('%Y-%m-%d %H:%M:%S')
        print(f"    [Trades] Buscando en API desde: {fecha_legible}")
        
        # Consultamos BTCUSDT como prueba
        raw_trades = client.get_my_trades(symbol='BTCUSDT', startTime=start_ts)
        print(f"    [API] Recibidos {len(raw_trades)} trades de BTCUSDT.")
