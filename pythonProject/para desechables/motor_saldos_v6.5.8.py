...
# ==========================================================
# 🟦 PROCESADOR BINGX (V6.5.9 - SOLUCIÓN CRYPTO + CFD)
# ==========================================================
# ==========================================================
# 🟦 PROCESADOR BINGX (V6.5.9 - INTEGRACIÓN MAESTRO V2.23)
# ==========================================================
def procesar_bingx(db, uid, ak, as_):
    ...
    def buscar_en_traductor_bingx(simbolo_api):
        """
        LÓGICA UNIFICADA V2.23:
        Limpia símbolos (quita guiones/slashes) y busca en la DB por 
        ticker_motor, ticker_limpio o underlying.
        """
        if not simbolo_api: return None
        
        # 1. Limpieza espejo a tu Motor Maestro
        tk_limpio = simbolo_api.upper().replace("-", "").replace("/", "").replace("=X", "")
        underlying_posible = tk_limpio.replace("USDT", "").replace("USDC", "")

        # 2. Consulta SQL de Triple Match (Directo, Limpio o Underlying)
        ...
        cursor.execute(sql, (simbolo_api, tk_limpio, underlying_posible))
        res = cursor.fetchone()
        
        # 3. Rescate para CFDs (AAPL, TSLA) si lo anterior falla
        if not res:
            sql_cfd = """SELECT id, ticker_motor, underlying, categoria_producto, tipo_investment 
                         ...
                         AND is_active = 1 LIMIT 1"""
            cursor.execute(sql_cfd, (tk_limpio,))
            res = cursor.fetchone()
            
        return res

    # --- 1. PROCESAR SALDOS SPOT (BTC, ETH, etc.) ---
    ...
    for b in balances:
        ...
        if total > 0.000001:
            asset = b.get('asset')
            # Buscamos en traductor para tener el ID y el precio
            info = buscar_en_traductor_bingx(asset)
            registrar_saldo(cursor, uid, info, total, float(b.get('locked', 0)), asset, "BINGX", "SPOT")
            s_count += 1
    print(f"    [OK] BingX Saldos Spot actualizado: {s_count} activos.")
    ...
    for o in res_os.get("data", {}).get("orders", []):
        ...
        if info:
            sql = """INSERT INTO sys_open_orders_spot 
                     (id_order_ext, user_id, broker_name, traductor_id, symbol, side, type, price, qty, locked_amount, fecha_utc, estado, last_seen) 
                     VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,NOW())"""
            cursor.execute(sql, (str(o.get('orderId')), uid, "BINGX", info['id'], symbol, o.get('side'), o.get('type'), 
                                 float(o.get('price', 0)), float(o.get('origQty', 0)), float(o.get('origQuoteOrderQty', 0)), 
                                 datetime.fromtimestamp(o.get('time', 0)/1000).strftime('%Y-%m-%d %H:%M:%S'), 'ABIERTA'))

    # --- 3. OPEN ORDERS FUTURES (BTC-USDT, XAUT-USDT, CFDs) ---
    ...
    for o in res_of.get("data", {}).get("orders", []):
        symbol = o.get('symbol')
        info = buscar_en_traductor_bingx(symbol)
        if info:
            sql = """INSERT INTO sys_open_orders 
                     (id_order_ext, user_id, broker_name, traductor_id, symbol, side, type, price, qty, locked_amount, fecha_utc, estado, last_seen) 
                     VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,NOW())"""
            cursor.execute(sql, (str(o.get('orderId')), uid, "BINGX", info['id'], symbol, o.get('side'), 'LIMIT', 
                                 float(o.get('price', 0)), float(o.get('origQty', 0)), 0.0, 
                                 datetime.fromtimestamp(o.get('updateTime', 0)/1000).strftime('%Y-%m-%d %H:%M:%S'), 'ABIERTA'))
            print(f"    [OK] Orden Futuros registrada: {symbol} (Asociada a ID:{info['id']})")
        else:
            print(f"    [ADVERTENCIA] No se pudo mapear '{symbol}'.")
...
# ==========================================================
def run():
    print(f"💎 MOTOR v6.5.7 - SALDOS + TRADES BINANCE-BINGX")
    while True:
