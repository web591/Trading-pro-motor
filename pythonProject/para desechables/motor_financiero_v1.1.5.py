...
def obtener_traductor_id(cursor, motor_fuente, ticker):
    ...
    res = cursor.fetchone()
    if not res:
        print(f"        [?] Sin traductor para: {ticker} ({motor_fuente})")
    return res
...
def registrar_cashflow(cursor, data):
    ...
    # Log de registro individual si es relevante (opcional, puede llenar la terminal)
    # print(f"        [REC] {data['tipo_evento']} | {data['asset']} | Cant: {data['cantidad']} | USD: {valor_usd:.2f}")
...
def ejecutar_motor_financiero(db):
    print(f"\n{'='*60}")
    print(f"💎 MOTOR FINANCIERO v1.1.5 - AUDITORÍA DE FLUJOS")
    print(f"{'='*60}")
    ...
    try:
        ...
        for u in usuarios:
            ...
            if not k or not s:
                # El print ya se hace dentro de descifrar_dato si falla
                continue
...
# ==========================================================
# 🚀 BLOQUE DE ARRANQUE (AÑADE ESTO AL FINAL)
# ==========================================================
if __name__ == "__main__":
    ...
    db = None
    try:
        # Conexión usando tu archivo config.py
