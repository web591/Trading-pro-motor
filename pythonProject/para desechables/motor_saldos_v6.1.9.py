...
def obtener_punto_inicio_sincro(cursor, uid, broker, endpoint):
    ...
    row = cursor.fetchone()
    # Si no hay registro, empezamos el 31.01.2026
...
# ==========================================================
# 📥 REGISTRO DE TRADES (v6.1.9 - Columnas SQL Validadas)
# ==========================================================
def registrar_trade_completo(cursor, uid, t_data):
    try:
        ...
        print(f"        [STRATEGY] Guardado: {t_data['symbol']} | ID: {t_data['orderId']} | Monto: {t_data['quoteQty']}")
    except Exception as e:
        print(f"        [!] Error DB Trade: {e}")

# ==========================================================
# 🟨 PROCESADOR BINANCE (Dual: Saldos + Diccionario)
# ==========================================================
def procesar_binance(db, uid, k, s):
    try:
        ...
        # --- 1. RADAR ORIGINAL (Saldos Actuales) ---
        ...
        for b in acc['balances']:
            ...
            if total > 0.000001:
                ...
                activos_con_saldo += 1
        print(f"    [OK] Radar Original: {activos_con_saldo} activos con saldo actualizados.")

        # --- 2. HISTORIAL POR DICCIONARIO ---
        ...
        print(f"    [Radar Diccionario] Analizando {len(diccionario)} pares configurados desde {datetime.fromtimestamp(start_ts/1000)}...")
        ...
        for item in diccionario:
            ...
            try:
                ...
                if raw_trades:
                    print(f"    [API] Recibidos {len(raw_trades)} trades de {pair}")
                    for t in raw_trades:
                        t_f = {
...
# ==========================================================
# 🟦 PROCESADOR BINGX (Saldos)
# ==========================================================
def procesar_bingx(db, uid, ak, as_):
    try:
        cursor = db.cursor(dictionary=True)
        # Aquí va tu lógica de BingX que ya funcionaba
        print(f"    [OK] BingX User {uid} procesado correctamente.")
    except Exception as e: print(f"    [!] Error BingX User {uid}: {e}")

# ==========================================================
# 🚀 EJECUCIÓN MAESTRA CON CIERRE DE CICLO
# ==========================================================
def run():
    print(f"💎 MOTOR v6.1.9 - AUDITORÍA COMPLETA Y LÓGICA DUAL")
    while True:
        print(f"\n{'='*60}")
        print(f"🔄 INICIANDO RECORRIDO DE ACTUALIZACIÓN: {datetime.now().strftime('%H:%M:%S')}")
        print(f"{'='*60}")
        ...
        try:
            ...
            for u in users:
                print(f"\n>> PROCESANDO: User {u['user_id']} | Broker: {u['broker_name']}")
                k = descifrar_dato(u['api_key'], MASTER_KEY)
                s = descifrar_dato(u['api_secret'], MASTER_KEY)
                
                if not k or not s:
                    print(f"    [!] Error: No se pudo descifrar las llaves para este usuario.")
                    continue
        ...
        print(f"💤 Entrando en pausa (5 min) hasta el próximo recorrido...")
        print(f"{'='*60}")
        time.sleep(300)
