...
def procesar_bingx(db, uid, ak, as_):
    ...
    def buscar_en_traductor_bingx(simbolo_api):
        ...
        # 1. Match en BingX (Cualquier motor bingx_%)
        # 2. Match en Binance (Cualquier motor binance_%)
        # 3. Match por Underlying en cualquier motor activo
        sql = """SELECT id, ticker_motor, underlying, categoria_producto, tipo_investment, motor_fuente 
                 FROM sys_traductor_simbolos 
                 WHERE is_active = 1 
                 AND (
                    (motor_fuente LIKE 'bingx_%%' AND (ticker_motor = %s OR REPLACE(ticker_motor, '-', '') = %s OR underlying = %s))
                    OR 
                    (motor_fuente LIKE 'binance_%%' AND (ticker_motor = %s OR REPLACE(ticker_motor, '-', '') = %s))
                    OR
                    (underlying = %s)
                 ) 
                 ORDER BY 
                    (motor_fuente LIKE 'bingx_%%') DESC, 
                    (motor_fuente LIKE 'binance_%%') DESC,
                    (ticker_motor = %s) DESC 
                 LIMIT 1"""
        
        cursor.execute(sql, (s_api, s_limpio, und_api, s_api, s_limpio, und_api, s_api))
        return cursor.fetchone()
    ...
    # --- 3. OPEN ORDERS FUTURES (CON REGISTRO FORZOSO) ---
    ...
    for o in res_of.get("data", {}).get("orders", []):
        ...
        # Si no hay info, usamos valores por defecto para no perder la orden
        ...
        if info:
            ...
            print(f"    [AVISO] Orden {sym} registrada SIN traductor (Fallback).")
