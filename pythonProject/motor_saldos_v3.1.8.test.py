...
def obtener_id_traductor(cur, symbol, user_id):
    ...
    try:
        cur.execute("INSERT IGNORE INTO sys_busqueda_resultados (ticker, motor, user_id, fecha_deteccion) VALUES (%s, 'sistema', %s, NOW())", (limpio, user_id))
    except: pass
    return None

# --- MOTOR BINANCE v3.1.8 ---
def motor_binance_v318(k, s, user_id, db):
    try:
        client = Client(k, s)
        cur = db.cursor(dictionary=True, buffered=True)
        
        # 1. SALDOS (Esto ya funciona perfecto)
        ...
        for b in acc['balances']:
            ...
            if tot > 0.000001:
                tid = obtener_id_traductor(cur, b['asset'], user_id)
        ...
        # 2. ÓRDENES (Cambiamos 'amount' por 'cantidad' o similar)
        cur.execute("DELETE FROM sys_open_orders WHERE user_id=%s", (user_id,))
        open_orders = client.get_open_orders()
        for o in open_orders:
            try:
                # Intentamos con 'cantidad' que es lo que sugiere tu PDF de trades
                cur.execute("""
                    INSERT IGNORE INTO sys_open_orders (user_id, symbol, side, price, cantidad)
                    VALUES (%s, %s, %s, %s, %s)
                """, (user_id, o['symbol'], o['side'], o['price'], o['origQty']))
            except mysql.connector.Error:
                # Si falla, insertamos lo mínimo para no romper el ciclo
                cur.execute("INSERT IGNORE INTO sys_open_orders (user_id, symbol, side, price) VALUES (%s, %s, %s, %s)", (user_id, o['symbol'], o['side'], o['price']))
...
# --- MOTOR BINGX v3.1.8 ---
def motor_bingx_v318(k, s, user_id, db, session):
    try:
        cur = db.cursor(dictionary=True, buffered=True)
...
if __name__ == "__main__":
    ...
    print("🚀 MOTOR v3.1.8 - FINALIZANDO AJUSTES")
    while True:
        try:
            ...
            for u in keys:
                ...
                if k_d and s_d:
                    if 'binance' in u['broker_name'].lower(): motor_binance_v318(k_d, s_d, u['user_id'], db)
                    elif 'bingx' in u['broker_name'].lower(): motor_bingx_v318(k_d, s_d, u['user_id'], db, session)
            db.commit(); db.close()
