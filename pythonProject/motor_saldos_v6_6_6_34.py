...
# ==========================================================
# 🟦 PROCESADOR BINGX (V6.8.5 - INTEGRADO CON V6.6.7)
# ==========================================================
def procesar_bingx(db, uid, ak, as_):
    ...
    # Procesar FUTUROS (Standard + Perpetual unificados en balance)
    try:
        res_perp = bx_req("/openApi/swap/v2/user/balance")
        bal = res_perp.get("data", {}).get("balance", {})
        if bal and float(bal.get("balance", 0)) > 0.000001:
            # v6.6.7 - Aunque usemos "bingx_futures", el rescate elástico lo vinculará 
            # al asset base (USDT) en el traductor.
    ...
    for conf in config_orders:
        ...
        for o in orders:
            sym = o.get('symbol')
            # v6.6.7 - Rescate elástico: bingx_futures -> bingx_spot/forex/stock
    ...
    try:
        ...
        for o in sorted(res_f.get("data", {}).get("orders", []), key=lambda x: x['updateTime']):
            ...
            # v6.6.7 - Aquí entra la magia: usará el motor_patron "bingx%"
            ...
            # Forzamos categoría FUTURES si el traductor dice CRYPTO/SPOT (porque vienen de API de Swap)
...
def procesar_binance_um_futures(db, uid, k, s):
    ...
    try:
        ...
        new_sync = max_ts_procesado + 1 if t_count > 0 else int(time.time()*1000)
        actualizar_punto_sincro(cursor, uid, "BINANCE", "trades_um_futures", new_sync)
        ...
        for item in diccionario:
            ...
            try:
                ...
                for t in sorted(trades, key=lambda x: x['time']):
                    ...
                    if registrar_trade(cursor, uid, t_f, item, "BINANCE"):
                        t_count += 1
                        if t['time'] > max_ts_procesado: max_ts_procesado = t['time']
            except: continue

        new_sync = max_ts_procesado + 1 if t_count > 0 else int(time.time()*1000)
        actualizar_punto_sincro(cursor, uid, "BINANCE", "trades_cm_futures", new_sync)
...
def ejecutar_ciclo_completo():
    print(f"💎 MOTOR v6.6.6.34 - SALDOS + TRADES + OPEN ORDERS + POSITION BINANCE-BINGX INSERT HOMOGENEOS")
