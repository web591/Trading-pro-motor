...
def mapeo_bingx(busqueda):
    ...
    try:
        # 1️⃣ OBTENER LISTADO DE CONTRATOS Y PRECIOS
        ...
        # 2️⃣ RECORRER CONTRATOS (FOREX, COMMODITY, STOCK, INDEX)
        for c in contratos:
            ...
            precio_data = precios_dict.get(sym)
            if not precio_data:
                continue
            ...
            # Clasificación
            if asset.startswith("NCFX"): universo = "FOREX"
            elif asset.startswith("NCCO"): universo = "COMMODITY"
            elif asset.startswith("NCSK"): universo = "STOCK"
            elif asset.startswith("NCSI"): universo = "INDEX"
            else: universo = "CRYPTO"

            nombre_limpio = underlying.replace("USDT","").replace("USD","").replace("PERP","").replace("2","")
        ...
        # 3️⃣ AGREGAR CRYPTO PURO (que no esté en contratos)
        for p in precios:
            sym = p.get("symbol","").upper()
            if not sym or sym in [x["Ticker"] for x in encontrados]:
                continue
