...
def obtener_traductor_id(cursor, ticker_base, categoria, broker, raw_ticker):
    """
    TRIANGULACIÓN: 
    Buscamos por ticker_motor (que es el símbolo puro como viene de Binance)
    """
    # En tu SQL, la columna es 'ticker_motor' y 'categoria_producto'
    # No usamos 'broker' ni 'broker_name' en el WHERE si no existen.
    
    query = """
        SELECT id FROM sys_traductor_simbolos 
        WHERE ticker_motor = %s 
          AND categoria_producto = %s
        LIMIT 1
    """
    
    try:
        cursor.execute(query, (raw_ticker, categoria))
        res = cursor.fetchone()
        
        if res:
            return res['id']
        else:
            # Si no lo encuentra con categoría, buscamos solo por el ticker
            cursor.execute("SELECT id FROM sys_traductor_simbolos WHERE ticker_motor = %s LIMIT 1", (raw_ticker,))
            res_fallback = cursor.fetchone()
            return res_fallback['id'] if res_fallback else None
            
    except Exception as e:
        print(f" [!] Error en obtener_traductor_id: {e}")
        return None
    ...
    try:
        ...
        for b in balances:
            ...
            if total > 0.00001:
                ...
                # Normalización y obtención de ID del traductor
                ...
                # SQL CORREGIDO: 
                # La tabla se llama sys_saldos_usuarios
                # Las columnas son: user_id, broker, tipo_cuenta, asset, cantidad_total, traductor_id
                
                # SQL CORREGIDO: Eliminamos la columna 'broker' y 'tipo_cuenta' si no existen
                # Solo insertamos lo que tu tabla sys_saldos_usuarios tiene realmente
                sql = """
                    ...
                    ON DUPLICATE KEY UPDATE 
                        ...
                        last_update = NOW()
                """
                # Ejecutamos con 4 parámetros (user_id, raw_asset, total, t_id)
                cursor.execute(sql, (user_id, raw_asset, total, t_id))

        conexion_db.commit()
        print(f" [+] Usuario {user_id}: {len(activos_encontrados)} activos sincronizados correctamente.")
        
        # Disparamos el radar para lo que no esté en seguimiento
        if activos_encontrados:
            ejecutar_radar_gema(conexion_db, user_id, activos_encontrados)

    except Exception as e:
        # Esto nos dirá exactamente en qué columna falla si persiste
        print(f" [!] Error en User {user_id}: {e}")

def iniciar_motor():
    print(f"\n{'='*40}\n GEMA v4.0.8 - MULTI-BROKER INTELLIGENT RADAR\n{'='*40}")
    while True:
        try:
