...
def mapeo_bingx(busqueda):
    ...
    # 2️⃣ CFD UNIVERSO
    cfd = buscar_swap_cfd_normalizado(tk)
    if cfd:
        encontrados.extend(cfd)

    # 3️⃣ TOKENIZED FALLBACK (STOCKS / INDICES)
    if not cfd:
        token = buscar_tokenizados_bingx(tk)
        if token:
            encontrados.extend(token)
