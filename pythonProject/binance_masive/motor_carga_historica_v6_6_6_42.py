...
def obtener_traductor_id_universal(cursor, motor_fuente, ticker_api):
    ...
    # ---------------------------------------------------------
    # PASO 1: BÚSQUEDA EXACTA - CORREGIDO (Añadido motor_fuente)
    # ---------------------------------------------------------
    sql_exacto = """
        SELECT id, ticker_motor, categoria_producto, tipo_investment, underlying, quote_asset, motor_fuente
    ...
    # ---------------------------------------------------------
    # SQL DE RESCATE - CORREGIDO (¡Aquí también faltaba motor_fuente!)
    # ---------------------------------------------------------
    sql_rescate = """
        SELECT id, ticker_motor, categoria_producto, tipo_investment, underlying, quote_asset, motor_fuente
        FROM sys_traductor_simbolos 
...
def procesar_bingx(db, uid, ak, as_):
    ...
    try:
        ...
        data_s = res_spot.get("data")
        balances = data_s.get("balances", []) if isinstance(data_s, dict) else []
        for b in balances:
        ...
        data_p = res_perp.get("data")
        bal = data_p.get("balance", {}) if isinstance(data_p, dict) else {}
    ...
    for conf in config_orders:
        ...
        data_o = res.get("data")
        orders_list = data_o.get("orders", []) if isinstance(data_o, dict) else []
    ...
    try:
        ...
        while current_start < ahora_ms:
            ...
            data_f = res_f.get("data")
            orders = data_f.get("orders", []) if isinstance(data_f, dict) else []
...
def procesar_bingx_positions(db, uid, ak, as_):
    ...
    try:
        ...
        pos_raw = res.get("data")
        posiciones = pos_raw if isinstance(pos_raw, list) else []
        p_count = 0
...
# ==========================================================
# 🟦 PROCESADOR BINANCE UM FUTURES - REPARADO (ESTILO v24.1)
# ==========================================================
def procesar_binance_um_futures(db, uid, k, s):
    ...
    try:
        ...
        # 2. TRADES HISTÓRICOS UM CON BLOQUES Y SEGURIDAD REFORZADA
        start_ts = obtener_punto_inicio_sincro(cursor, uid, "BINANCE", "trades_um_futures")
        punto_rastreo = start_ts - 60000 
        ahora_ms = int(time.time() * 1000)
        ventana_7d = 7 * 24 * 60 * 60 * 1000 

        cursor.execute("SELECT id, ticker_motor, categoria_producto, tipo_investment, motor_fuente FROM sys_traductor_simbolos WHERE motor_fuente = 'binance_usdt_future'")
        diccionario = cursor.fetchall()
        t_count = 0

        for item in diccionario:
            symbol = item['ticker_motor']
            print(f"      [*] Analizando UM: {symbol}...")
            current_start = punto_rastreo
            
            while current_start < ahora_ms:
                # Margen de seguridad: 1 minuto menos que los 7 días
                current_end = min(current_start + ventana_7d - 60000, ahora_ms)

                try:
                    trades = client.get_account_trades(symbol=symbol, startTime=current_start, endTime=current_end)
                    
                    if trades:
                        print(f"        [OK] Bloque UM {symbol}: {len(trades)} encontrados.")
                    ...
                    # Avanzamos si la petición fue exitosa
                    current_start = current_end + 60000 # Saltamos al siguiente bloque
                    time.sleep(1.0) # Pausa obligatoria entre peticiones para control de Weight

                except Exception as e:
                    error_str = str(e).upper()
                    if "429" in error_str or "-1003" in error_str or "LIMIT" in error_str:
                        print(f"⚠️ [RATE LIMIT UM] IP Agotada en {symbol}. Esperando 45 segundos...")
                        time.sleep(45) 
                        continue # Reintenta el mismo bloque
                    else:
                        print(f"      [!] Error en UM {symbol}: {e}")
                        current_start = current_end + 60000 
                        break # Salta al siguiente símbolo si es un error distinto a Rate Limit
...
# ==========================================================
# 🟧 PROCESADOR BINANCE CM FUTURES - REPARADO (ESTILO v24.1)
# ==========================================================
def procesar_binance_cm_futures(db, uid, k, s):
    ...
    try:
        ...
        cursor.execute("SELECT id, ticker_motor, categoria_producto, tipo_investment, motor_fuente FROM sys_traductor_simbolos WHERE motor_fuente = 'binance_coin_future'")
        diccionario = cursor.fetchall()
        t_count = 0
        ...
        for item in diccionario:
            ...
            while current_start < ahora_ms:
                ...
                try:
                    ...
                    # Si la petición tuvo éxito, avanzamos el tiempo
                    current_start = current_end
                    # Pausa obligatoria entre bloques para no saturar
                    time.sleep(1.5) 

                except Exception as e:
                    error_str = str(e).upper()
                    if "429" in error_str or "-1003" in error_str or "LIMIT" in error_str:
                        print(f"⚠️ [RATE LIMIT CM] IP Agotada en {symbol}. Esperando 45 segundos...")
                        time.sleep(45) # Pausa larga para limpiar el weight de la IP
                        continue # Reintenta el MISMO bloque (no avanza current_start)
                    else:
                        print(f"      [!] Error NO recuperable en CM {symbol}: {e}")
                        # En caso de error raro (ej. símbolo inexistente), saltamos bloque para no encallarnos
                        current_start = current_end 
                        break 
...
def ejecutar_ciclo_completo():
    print(f"💎 MOTOR CARGA HISTORICA v6.6.6.41 - HIBRIDACIÓN CSV")
    ...
    try:
        ...
        usuarios = cursor.fetchall() # Guardamos los resultados primero

        for u in usuarios:
            uid = u['user_id']
            db = check_db_connection(db)
            
            # Desciframos las credenciales una sola vez por usuario
            k = descifrar_dato(u['api_key'], MASTER_KEY)
            s = descifrar_dato(u['api_secret'], MASTER_KEY)
            
            print(f"\n>> TRABAJANDO: User {uid} | {u['broker_name']}")

            if u['broker_name'].upper() == "BINANCE":
                # 1. CARGA HISTÓRICA POR CSV (Solo para el usuario que tiene los archivos)
                # Ponemos el IF aquí dentro para que uid ya esté definido
                if uid == 6:
                    ingestor_hibrido_csv(db, uid)

                # 2. CONTINÚA LA API NORMAL
                db = check_db_connection(db)
                print("        >>> SPOT BINANCE <<<")
                procesar_binance(db, uid, k, s)

                db = check_db_connection(db)
                print("        >>> UM FUTURES BINANCE <<<")
                procesar_binance_um_futures(db, uid, k, s)
                procesar_binance_um_positions(db, uid, k, s)

                db = check_db_connection(db)
                print("        >>> CM FUTURES BINANCE <<<")
                procesar_binance_cm_futures(db, uid, k, s)
                procesar_binance_cm_positions(db, uid, k, s)

            elif u['broker_name'].upper() == "BINGX":
                db = check_db_connection(db)
                print("        >>> BINGX <<<")
                procesar_bingx(db, uid, k, s)
                procesar_bingx_positions(db, uid, k, s)

            # 🚀 NORMALIZADOR GLOBAL
            db = check_db_connection(db)
            print(f"        >>> 🧹 SWEEPER DE COMISIONES (USER: {uid}) <<<")
            normalizar_comisiones_pendientes(db, uid)
