...
def limpiar_ticker_profesional(symbol):
    """Protege TUSD/USDP y quita basura"""
...
def disparador_radar(cur, symbol):
    """Propuesta 6: Todo a sys_simbolos_buscados"""
    ticker_limpio = limpiar_ticker_profesional(symbol)
    if not ticker_limpio: return
    try:
        cur.execute("INSERT IGNORE INTO sys_simbolos_buscados (ticker, status) VALUES (%s, 'pendiente')", (ticker_limpio,))
    except: pass
...
# --- BINANCE: Tabla sys_open_orders ---
def procesar_binance(key, sec, user_id, db):
    try:
...
# --- BINGX: Tabla sys_open_orders_bingx ---
def procesar_bingx(key, sec, user_id, db, session):
    try:
        ...
        # Órdenes Spot -> sys_open_orders_bingx
        cur.execute("DELETE FROM sys_open_orders_bingx WHERE user_id=%s", (user_id,))
        # Intentamos obtener órdenes abiertas de USDT para el radar
        res_oo = bx_req("/openApi/spot/v1/trade/openOrders")
        if res_oo.get('code') == 0:
            for o in res_oo.get('data', []):
                cur.execute("INSERT INTO sys_open_orders_bingx (user_id, symbol, side, type, price, amount, status, fecha_utc) VALUES (%s,%s,%s,%s,%s,%s,%s,FROM_UNIXTIME(%s/1000))", (user_id, o['symbol'], o['side'], o['type'], o['price'], o['origQty'], 'NEW', o['time']))
                disparador_radar(cur, o['symbol'])
...
if __name__ == "__main__":
    ...
    print("🚀 MOTOR v2.2.5 - TABLAS: sys_open_orders & sys_open_orders_bingx")
    while True:
        try:
            ...
            for u in users:
                ...
                if 'binance' in u['broker_name'].lower(): procesar_binance(k, s, u['user_id'], db)
                elif 'bingx' in u['broker_name'].lower(): procesar_bingx(k, s, u['user_id'], db, session)
            db.close(); ciclo += 1
            print(f"✅ Ciclo {ciclo} OK."); time.sleep(ESPERA_CICLO_RAPIDO)
