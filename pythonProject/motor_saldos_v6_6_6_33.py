...
def procesar_binance(db, uid, k, s):
    ...
    try:
        ...
        for oo in open_orders:
            ...
            if not info:
                # 🚨 CAMBIO: En lugar de solo print, insertamos tarea
                ...
                continue # No registramos el trade hasta que exista el ID
...
# ==========================================================
# 🟦 PROCESADOR BINANCE UM FUTURES (USDT-M) - v6.6.7.05 (FIXED)
# ==========================================================
# ==========================================================
# 🟦 PROCESADOR BINANCE UM FUTURES - REPARADO (ESTILO v24)
# ==========================================================
def procesar_binance_um_futures(db, uid, k, s):
    asset_actual = "INICIO" # Para evitar el error de variable no definida
    try:
        ...
        for b in acc:
            ...
            if total > 0.000001:
                # Buscamos el ID, pero si no existe (como en USDT), NO matamos el proceso
                info = obtener_traductor_id_universal(cursor, "binance_usdt_future", asset_actual)
                tid = info['id'] if info else None
                
                # Registramos el saldo tal cual, permitiendo tid=None si es la moneda base (USDT/USDC)
                registrar_saldo(cursor, uid, info, total, 0.0, asset_actual, "BINANCE", "FUTURES")
                s_count += 1
        
        print(f"    [UM] Saldos: {s_count}")

        # 2. TRADES (Aquí sí es vital el traductor_id)
        # ... resto del código de trades igual ...

    except Exception as e:
        # Si el error fue por la columna de tareas, es porque el nombre es symbol_detectado
        print(f"    [UM ERROR en {asset_actual}] {e}")

        # 2. TRADES HISTÓRICOS UM
        start_ts = obtener_punto_inicio_sincro(cursor, uid, "BINANCE", "trades_um_futures")
        
        # Eliminado 'is_active = 1' para que lea TODOS los símbolos del traductor
        cursor.execute("""
        ...
        for item in diccionario:
            symbol = item['ticker_motor'] # Aquí sí definimos symbol
            try:
                ...
                continue # Si un símbolo falla, pasamos al siguiente sin romper el motor
...
def procesar_binance_cm_futures(db, uid, k, s):
    ...
    try:
        ...
        # Limpiamos solo lo de este usuario y broker
        ...
        for b in acc:
            ...
            if total > 0.000001:
                # Buscamos ID pero no bloqueamos si no hay
                info = obtener_traductor_id_universal(cursor, "binance_coin_future", asset_actual)
                
                # registrar_saldo ya maneja si info es None (pone NULL en la DB)
        ...
        for row in simbolos:
            ...
            try:
                ...
                for oo in abiertas:
                    ...
                    if not info:
                        tid = None
                        # 🚨 CAMBIO: Generar tarea automática
                        generar_tarea_incorporacion(cursor, uid, "BINANCE", sym, "OPEN_ORDER_NUEVA")
                    else:
                        tid = info['id']
...
def ejecutar_ciclo_completo():
    print(f"💎 MOTOR v6.6.6.33 - SALDOS + TRADES + OPEN ORDERS + POSITION BINANCE-BINGX INSERT HOMOGENEOS")
