...
def binance_deposits(db, user_id, key, secret):
    ...
    # Arrancar desde el inicio real si está en 0 o resetear manual
    start_ts = last_sync + 1 if last_sync > 0 else 1664582400000 # Octubre 2022
    end_now = int(time.time() * 1000)
    ...
    while start_ts < end_now:
        ...
        try:
            response = requests.get(url, headers={"X-MBX-APIKEY": key})
            r = response.json()
        except Exception as e:
            print(f"\n        [!] Error de conexión en bloque {fecha_h}: {e}. Reintentando en próximo ciclo...")
            break # Detiene el avance para no saltarse este bloque en la BD

        # Validar que sea una lista real antes de avanzar el cursor de la base de datos
        if isinstance(r, list):
            for d in r:
                ...
                if "Received from" in raw_str or "Paid to" in raw_str:
                    evento_assigned = "TRANSFER_IN"
                else:
                    evento_assigned = "DEPOSIT"

                registrar_cashflow(cursor, {
                    "user_id": user_id,
                    "broker": "BINANCE",
                    "tipo_evento": evento_assigned,
                    "asset": d["coin"],
                    "cantidad": float(d["amount"]),
                    "ticker_motor": None,
                    "fecha": time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(ts/1000)),
            ...
            # El avance de la sincronización en la BD SOLO se guarda si el bloque se procesó exitosamente
            guardar_sync(cursor, user_id, "BINANCE", endpoint, chunk_end)
            db.commit()
            start_ts = chunk_end + 1
        else:
            # Si Binance devolvió un error (ej. Rate limit o IP bloqueada temporalmente)
            print(f"\n        [!] Binance API Error en bloque {fecha_h}: {r}")
            break # SE DETIENE. No actualiza start_ts ni guarda sync para reintentar este bloque exacto después.
...
def binance_withdraw(db, user_id, key, secret):
    ...
    while start_ts < end_now:
        ...
        try:
            ...
            print(f"\n        [!] Error de conexión en bloque {fecha_h}: {e}")
            break
        ...
        if isinstance(r, list):
            for w in r:
                ...
                if "Received from" in raw_str or "Paid to" in raw_str:
                    evento_assigned = "TRANSFER_OUT"
                else:
                    evento_assigned = "WITHDRAW"
...
# ==========================================================
# ⛏️ BLOQUE: BINANCE MINING (v1.3.10 - Fusión Histórica Blindada)
# ==========================================================
def binance_mining(db, user_id, key, secret):
    ...
    # 🔄 MATRIZ CRUZADA DE EXTRACCIÓN
    for account in mining_accounts:
        for algo in algoritmos:
            start_ts = start_ts_base
            
            while start_ts < end_now:
                # ✅ Bloques seguros de 90 días para la API de Binance Pool
                chunk_end = start_ts + (90 * 24 * 60 * 60 * 1000)
                if chunk_end > end_now:
                    chunk_end = end_now
                ...
                try:
                    ...
                    if "data" in r and "accountProfits" in r["data"]:
                        ...
                        # 🛡️ VALIDACIÓN CORRECTA DE INTEGRIDAD: 
                        # Si Binance nos devuelve datos que pertenecen a otro bloque temporal (del futuro/presente),
                        # saltamos este bloque en lugar de romper el script completo con un 'break'.
                        primer_ts = int(profits[0]["time"])
                        if primer_ts > chunk_end:
                            start_ts = chunk_end + 1
                            continue
                ...
                # Avanzamos al siguiente bloque de 90 días de forma estricta
                start_ts = chunk_end + 1
                rate_limit(0.2)

            # Confirmar cambios en la DB al terminar cada combinación cuenta/algo
            if count_total > 0:
                db.commit()
...
def ejecutar_motor_financiero(db):
    ...
    print(f"\n{'='*60}")
    print(f"💎 MOTOR FINANCIERO v1.3.11 - AUDITORÍA Y DIVIDENDOS")
    print(f"{'='*60}")
