...
def registrar_trade_completo(cursor, uid, t_data):
    try:
        # ID Único para evitar duplicidad entre usuarios
        id_vinculo = f"{uid}-{t_data['orderId']}"
        
        # 1. INSERT EN TRANSACCIONES_GLOBALES (Contabilidad)
        sql_global = """
        ...
        cursor.execute(sql_global, (
            id_vinculo, uid, 'BINANCE', 'SPOT', 'TRADE', 
            t_data['symbol'], t_data['quoteQty'], t_data['commission'], 
            t_data['fecha_sql'], 'BINANCE'
        ))
        ...
        # Convertimos el diccionario original a texto para el backup
        raw_json_str = json.dumps(t_data)
        
        valores_detalle = (
            uid, 'BINANCE', 'SPOT', 'BINANCE', 'SPOT',
            id_vinculo, 
            t_data['fecha_sql'], 
            t_data['symbol'], 
            t_data['side'], 
            t_data['price'], 
            t_data['qty'],
            t_data['commission'],      # <--- NUEVO
            t_data['commissionAsset'], # <--- NUEVO
            t_data['quoteQty'],        # <--- NUEVO
            1 if t_data.get('isMaker') else 0, # <--- NUEVO
            'BINANCE', 
            f"TRD-{id_vinculo}",
            raw_json_str               # <--- NUEVO: El respaldo total
        )
...
# ==========================================================
def run():
    print(f"💎 MOTOR v6.3.1 - MULTI-BROKER (PRODUCCIÓN)")
    while True:
