...
def tarea_binance(key, sec, user_id, db):
    try:
        ...
        # 2. FUTUROS (Mapeo Extendido)
        f_acc = client.futures_account()
        for f in f_acc['assets']:
            wb = float(f['walletBalance'])
            if wb > 0:
                upnl = float(f['unrealizedProfit'])
                m_maint = float(f.get('maintMargin', 0)) # Margen de mantenimiento
                m_init = float(f.get('initialMargin', 0)) # Margen usado/inicial
                m_avail = float(f.get('marginBalance', 0)) - m_init # Disponible estimado
                eq = wb + upnl
...
def tarea_bingx(key, sec, user_id, db, session):
    try:
        ...
        # 2. PERPETUOS BINGX (Mapeo del JSON que pegaste)
