...
def obtener_precio_db(cursor, asset):
    ...
    # Limpieza solo para búsqueda de precio
...
def tarea_binance(key, sec, user_id, db):
    try:
        ...
        # 1. SPOT & EARN (Incluyendo prefijos LD)
        ...
        for b in acc['balances']:
            ...
            if total > 0:
                ...
                cur.execute("""INSERT INTO sys_saldos_usuarios 
                    ...
                    (user_id, 'Binance', 'SPOT', b['asset'], total, f, l, p, v_usd))
        ...
        for f in f_acc['assets']:
            ...
            if wb > 0:
                ...
                cur.execute("""INSERT INTO sys_saldos_usuarios 
                    ...
                    (user_id, 'Binance', 'PERPETUAL', f['asset'], wb, upnl, eq, eq))
        db.commit()
        print(f"   ✅ Binance ID {user_id}: Sincronizado (Spot + Futuros)")
        ...
        if res_spot.get('code') == 0:
            for b in res_spot['data']['balances']:
                ...
                if total > 0:
                    p = obtener_precio_db(cur, b['asset'])
                    cur.execute("""INSERT INTO sys_saldos_usuarios 
                        (user_id, broker_name, tipo_cuenta, asset, cantidad_total, cantidad_disponible, valor_usd, last_update) 
                        VALUES (%s, %s, %s, %s, %s, %s, %s, NOW())""",
                        (user_id, 'BingX', 'SPOT', b['asset'], total, float(b['free']), total * p))

        # 2. PERPETUOS BINGX (Mapeo del objeto único 'balance')
        res_swap = bingx_req("/openApi/swap/v2/user/balance")
        if res_swap.get('code') == 0:
            d = res_swap['data']['balance']
            wb = float(d['balance'])
            if wb > 0:
                upnl = float(d['unrealizedProfit'])
                eq = float(d['equity'])
                cur.execute("""INSERT INTO sys_saldos_usuarios 
                    (user_id, broker_name, tipo_cuenta, asset, cantidad_total, pnl_no_realizado, equidad_neta, valor_usd, last_update) 
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, NOW())""",
                    (user_id, 'BingX', 'PERPETUAL', d['asset'], wb, upnl, eq, eq))

        db.commit()
        print(f"   ✅ BingX ID {user_id}: Sincronizado (Spot + Perpetuos)")
    except Exception as e: print(f" ❌ Error BingX ID {user_id}: {e}")

def motor():
    print("🚀 MOTOR v1.11 - CAPTURA TOTAL (No Dust Filter)")
