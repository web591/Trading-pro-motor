...
def tarea_bingx(key, sec, user_id, db, session):
    try:
        ...
        res_fund = bingx_request("/openApi/wallets/v1/capital/getAsset")
        if res_fund.get('code') == 0:
            for f in res_fund['data']:
                total = float(f['balance'])
                if total > 0.01:
                    p = obtener_precio_db(cur, f['asset'])
                    cur.execute("INSERT INTO sys_saldos_usuarios (user_id, broker_name, tipo_cuenta, asset, cantidad_total, valor_usd, last_update) VALUES (%s, %s, %s, %s, %s, %s, NOW())", 
                               (user_id, 'BingX', 'CASH', f['asset'], total, total * p))

        db.commit()
        print(f"  ✅ BingX ID {user_id} Sincronizado.")
    except Exception as e: 
