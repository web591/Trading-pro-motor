...
# ==========================================================
# BINGX UNIVERSO TOTAL TOKENIZED FIX
# Version 2.3 NAN PATCH
# ==========================================================
...
def mapeo_bingx(busqueda):
    ...
    try:
        ...
        for c in contratos:
            ...
            displayName = str(c.get("displayName")).upper()

            # --------------------------------------------------
            # 🔥 NAN PATCH REAL
            # --------------------------------------------------
            if not underlying or str(underlying).lower() == "nan":
                nombre_base = displayName.split("(")[0].replace("-USDT","").strip()
            else:
                nombre_base = str(underlying).replace("/","").upper()

            if tk not in nombre_base and tk not in displayName:
                continue
