    ...
    while True:
        try:
            ...
            if tarea:
                ...
                # BLOQUEO INMEDIATO: Marcamos como 'procesando'
                ...
                # Buscamos si ya existen resultados para este mismo nombre_comun
                # Usamos GROUP BY para no traer duplicados innecesarios
                ...
                if existentes:
                    print(f"🧠 MEMORIA: {tk_busqueda} ya está en caché. Sincronizando datos...")
                    ...
                    # --- PASO B: BÚSQUEDA FRESCA EN APIS 📡 ---
                    print(f"🔍 Interrogando mercados para {tk_busqueda}...")
                    consolidado = []
                    
                    # Lista de tus funciones de mapeo
                    motores = [
                ...
                # --- PAUSA DE ACCIÓN (10 SEGUNDOS) ---
                print(f"⏳ Tarea {id_tarea} finalizada. Pausa de 10s...")
                time.sleep(10)
            ...
            if err.errno == 1226:
                print("\n⚠️ Límite Hostinger superado. Pausa de 5 min...")
                time.sleep(300)
            else:
                print(f"\n❌ Error DB: {err}")
                conn = None
