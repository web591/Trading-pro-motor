...
# --- TRADUCTOR CORREGIDO SEGÚN PDF ---
def obtener_id_traductor(cur, symbol, motor_name, user_id):
    limpio = symbol.upper().replace('USDT', '').replace('USDC', '').replace('-', '')
    if limpio.startswith('LD'): limpio = limpio[2:]
    
    # PDF: La columna es nombre_comun
    cur.execute("SELECT id FROM sys_traductor_simbolos WHERE nombre_comun = %s LIMIT 1", (limpio,))
    res = cur.fetchone()
    if res: return res['id']
    
    # RADAR: Según PDF (ticker, motor, user_id, fecha_deteccion)
    try:
        cur.execute("""
            INSERT IGNORE INTO sys_busqueda_resultados (ticker, motor, user_id, fecha_deteccion) 
            VALUES (%s, %s, %s, NOW())
        """, (limpio, motor_name, user_id))
    except: pass
    return None

# --- MOTOR BINANCE v3.1.7 ---
def motor_binance_v317(k, s, user_id, db):
    try:
        client = Client(k, s)
        ...
        for b in acc['balances']:
            ...
            if tot > 0.000001:
                tid = obtener_id_traductor(cur, b['asset'], 'binance_spot', user_id)
                cur.execute("""
        ...
        # 2. ÓRDENES (Limpieza segura)
        cur.execute("DELETE FROM sys_open_orders WHERE user_id=%s", (user_id,))
        
        open_orders = client.get_open_orders()
        for o in open_orders:
            # Insert simple para evitar errores de columnas faltantes
            cur.execute("""
                INSERT IGNORE INTO sys_open_orders (user_id, symbol, side, price, amount)
                VALUES (%s, %s, %s, %s, %s)
...
# --- MOTOR BINGX v3.1.7 ---
def motor_bingx_v317(k, s, user_id, db, session):
    try:
        cur = db.cursor(dictionary=True, buffered=True)
        ...
        if res.get('code') == 0:
            ...
            for b in res['data']['balances']:
                tot = float(b['free']) + float(b['locked'])
                if tot > 0.000001:
                    tid = obtener_id_traductor(cur, b['asset'], 'bingx_spot', user_id)
                    cur.execute("""
                        INSERT INTO sys_saldos_usuarios (user_id, broker_name, asset, traductor_id, cantidad_total, last_update)
...
if __name__ == "__main__":
    ...
    print("🚀 MOTOR v3.1.7 - BASADO EN PDF 07-02")
    while True:
        try:
            ...
            for u in keys:
                ...
                if 'binance' in u['broker_name'].lower():
                    motor_binance_v317(k_d, s_d, u['user_id'], db)
                elif 'bingx' in u['broker_name'].lower():
                    motor_bingx_v317(k_d, s_d, u['user_id'], db, session)
