...
def procesar_bingx(db, uid, ak, as_):
    ...
    try:
        ...
        bal_p = res_perp.get("data", {}).get("balance", {})
        if bal_p and float(bal_p.get("balance", 0)) > 0:
            info = buscar_en_traductor_bingx(bal_p.get("asset"))
            registrar_saldo(cursor, uid, info, float(bal_p["balance"]), float(bal_p.get("freezedMargin", 0)), bal_p["asset"], "BINGX", "FUTURES")
        print(f"    [OK] BingX Saldos procesados.")
    except Exception as e: print(f"    [!] Error Saldos BingX: {e}")

    # 2. TRADES (CON PROTECCIÓN DE TIPO)
    ...
    for item in universo:
        ...
        # FIX: Validamos que 'data' sea una lista antes de procesar
        trades_raw = res_tr.get("data", [])
        if isinstance(trades_raw, list) and len(trades_raw) > 0:
            print(f"    [DEBUG API] BingX encontró {len(trades_raw)} trades en {sym}")
            for t in trades_raw:
                if isinstance(t, dict) and str(t.get('status', '')).upper() in ['FILLED', 'PARTIALLY_FILLED', 'CLOSED', 'COMPLETED']:
                    t_f = {
    ...
    print(f"    [INFO] BingX Trades: {t_count} nuevos.")

# ==========================================================
# 🟨 PROCESADOR BINANCE (V6.7.3 - DEBUG FUTUROS)
# ==========================================================
def procesar_binance(db, uid, k, s):
    try:
        ...
        # TRADES FUTUROS (CON DEBUG ESTRATÉGICO)
        mapping_futuros = [
        ...
        for config_f in mapping_futuros:
            ...
            for item in dict_fut:
                try:
                    ...
                    if f_trades and len(f_trades) > 0:
                        # ESTO ES LO QUE NECESITO VER EN TU CONSOLA:
                        print(f"    [DEBUG API] Binance {config_f['motor']} detectó {len(f_trades)} trades para {item['ticker_motor']}")
                        for t in f_trades:
                            t_f = {
                                'orderId': str(t['orderId']), 'symbol': t['symbol'], 
                                'side': 'BUY' if float(t.get('qty', t.get('accQty', 0))) > 0 else 'SELL',
                                'price': float(t['price']), 'qty': abs(float(t.get('qty', t.get('accQty', 0)))), 
                                'quoteQty': abs(float(t.get('quoteQty', 0))), 'commission': float(t['commission']), 
                                'commissionAsset': t['commissionAsset'], 'fecha_sql': datetime.fromtimestamp(t['time']/1000).strftime('%Y-%m-%d %H:%M:%S')
                            }
...
# ==========================================================
def run():
    print(f"💎 MOTOR v6.7.3 - DEBUG ACTIVO")
    while True:
