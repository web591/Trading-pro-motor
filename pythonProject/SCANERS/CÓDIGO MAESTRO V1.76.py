...
def mapeo_bingx(busqueda):
    ...
    # 1. ADN DE ACTIVOS: Mapeamos la búsqueda del usuario a los nombres de BingX
    mapeo_adn = {
        "GOLD": ["GOLD", "XAU"],
        "SILVER": ["SILVER", "XAG"],
        "DAX": ["DAX", "GER40", "GER30"],
        "OIL": ["CRCL", "WTI"], # 'CRCL' es Crude Oil en BingX
        "EURUSD": ["EUR", "EURUSD"],
        "USDMXN": ["MXN", "USDMXN"],
        "GBPUSD": ["GBP", "GBPUSD"]
    }
    
    # Si la búsqueda está en el mapa, usamos esa familia. Si no, usamos el ticker base.
    familia = mapeo_adn.get(tk, [tk])
    ...
    for nombre_mkt, url in mercados:
        try:
            ...
            for i in items:
                ...
                # Quitamos guiones y USDT para comparar "manzanas con manzanas"
                # Ej: 'EUR-USDT' se convierte en 'EUR'
                sym_clean = sym.replace("-USDT", "").replace("USDT", "").replace("-", "")
                
                # REGLA DE VALIDACIÓN:
                # Aceptamos si es match exacto (EUR == EUR) o si es un prefijo oficial (NCCOEUR, AAPLX)
                for obj in familia:
                    es_exacto = (sym_clean == obj)
                    es_sintetico = (sym_clean == f"{obj}X")
                    es_copy = (f"NCCO{obj}" in sym or f"NCSK{obj}" in sym)
                    
                    if es_exacto or es_sintetico or es_copy:
                        # Evitamos duplicados de memecoins (ej: si buscamos EUR y existe "EURODOGE")
                        # Esta validación de longitud ayuda a filtrar basura
                        if len(sym_clean) <= len(obj) + 2 or es_copy:
                            precio = i.get('lastPrice') or i.get('price')
                            encontrados.append({
                                "Motor": nombre_mkt,
                                "Ticker": sym,
                                "Precio": precio,
                                "Info": "Forex/Sintético BingX" if tk in mapeo_adn else "Activo Verificado"
                            })
                        break # Ya encontramos este símbolo, pasamos al siguiente item
        except: continue
...
def ejecutor_maestro_v1_6():
    print(f"💎 CÓDIGO MAESTRO V1.76 - FILTRADO FINNHUB FX/CRYPTO")
