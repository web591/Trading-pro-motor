...
def mapeo_bingx(busqueda):
    ...
    # 1. EL TRADUCTOR DE IDENTIDAD (El "Alma" del activo)
    # Esto permite que si buscas GOLD, el motor sepa que XAU y GOLD son lo mismo.
    # Pero si buscas AMZN (que no está aquí), usará AMZN por defecto.
    identidades = {
        "GOLD": ["GOLD", "XAU", "PAXG", "XAUT"],
        "SILVER": ["SILVER", "XAG"],
    ...
    # Si el ticker tiene "familia", buscamos todos los miembros. 
    # Si no, buscamos solo el ticker (Para Stocks como IBM, AAPL, AMZN...)
    familia_adn = identidades.get(tk, [tk])
    ...
    for nombre_mkt, url in mercados:
        try:
            ...
            for i in items:
                ...
                # LA PRUEBA DE ADN
                match_hallado = False
                for adn_raiz in familia_adn:
                    # Si el ADN está en el símbolo (ej: XAU en NCCOGOLD o EUR en EUR-USDT)
                    if adn_raiz in sym:
                        # Limpiamos para evitar falsos positivos (SOL vs GASOLINE)
                        # El ticker limpio NO debe ser mucho más largo que la raíz 
                        # a menos que tenga prefijos NC
                        sym_limpio = sym.replace("-USDT","").replace("USDT","").replace("-","").replace("NCFX","").replace("NCCO","").replace("NCSK","")
                        
                        if sym_limpio == adn_raiz or sym_limpio == f"{adn_raiz}X" or (adn_raiz in sym_limpio and len(sym_limpio) <= len(adn_raiz)+3):
                            match_hallado = True
                            break
...
def ejecutor_maestro_v1_6():
    print(f"💎 CÓDIGO MAESTRO V1.91 - FILTRADO FINNHUB FX/CRYPTO")
