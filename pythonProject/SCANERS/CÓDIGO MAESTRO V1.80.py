...
def mapeo_bingx(busqueda):
    ...
    # 1. ADN EXPANDIDO: Incluimos Stocks y sus variantes sintéticas
    mapeo_adn = {
        "GOLD": ["GOLD", "XAU", "NCCOGOLD", "XAUT", "PAXG"],
        "SILVER": ["SILVER", "XAG", "NCCOSILVER"],
        "DAX": ["DAX", "GER", "NVDAX", "DE30", "DE40"],
        "OIL": ["WTI", "OIL", "CRCL", "CRCLX"],
        "EURUSD": ["EUR", "EURUSD", "NCFXEUR"],
        "USDMXN": ["MXN", "USDMXN", "NCFXMXN"],
        # NUEVA SECCIÓN DE STOCKS:
        "AAPL": ["AAPL", "AAPLX", "NCCOAAPL"],
        "IBM": ["IBM", "IBMX", "NCCOIBM"],
        "TSLA": ["TSLA", "TSLAX", "NCCOTSLA"],
        "NVDA": ["NVDA", "NVDAX", "NCCONVDA"]
    }
    
    # Familia de búsqueda: si no está en el mapa, intentamos el ticker + variantes comunes
    familia = mapeo_adn.get(tk, [tk, f"{tk}X", f"NCCO{tk}"])
    ...
    for nombre_mkt, url in mercados:
        try:
            ...
            for i in items:
                ...
                # Limpieza base para comparación
                sym_fix = sym.replace("-USDT", "").replace("USDT", "").replace("-", "")
                
                es_valido = False
                
                # REGLA MAESTRA DE VALIDACIÓN
                # 1. Match exacto con cualquier miembro de la familia (AAPL, AAPLX, etc.)
                if any(miembro == sym_fix for miembro in familia):
                    es_valido = True
                
                # 2. Contención inteligente para sintéticos largos (ej: NCCOAAPL2USD)
                elif any(miembro in sym for miembro in familia if len(miembro) > 3):
                    # Solo si la búsqueda es específica de nuestro ADN
                    if tk in mapeo_adn:
                        es_valido = True
                ...
                if es_valido:
                    ...
                    if precio:
                        # Etiquetamos si es un sintético de BingX
                        info_tag = "Sintético/Stock BingX" if any(x in sym for x in ["X-", "NCCO", "NCSK"]) else "Activo Verificado"
                        encontrados.append({
                            "Motor": nombre_mkt,
                            "Ticker": sym,
                            "Precio": precio,
...
def ejecutor_maestro_v1_6():
    print(f"💎 CÓDIGO MAESTRO V1.80 - FILTRADO FINNHUB FX/CRYPTO")
