...
def mapeo_bingx(busqueda):
    ...
    # 1. DICCIONARIO DE TRADUCCIÓN (Lo que tú pides vs Lo que BingX ofrece)
    # Esto asegura que EURUSD encuentre el par correcto y no basura.
    traductor = {
        "EURUSD": ["EUR-USDT"],
        "USDMXN": ["MXN-USDT"],
        "GBPUSD": ["GBP-USDT"],
        "GOLD": ["NCCOGOLD2USD-USDT", "XAUT-USDT", "PAXG-USDT"],
        "DAX": ["NVDAX-USDT", "GER30-USDT", "GER40-USDT"],
        "OIL": ["CRCLX-USDT", "WTI-USDT", "NCSKCRCL2USD-USDT"]
    }
    
    # 2. Definimos la familia de búsqueda
    # Si está en el traductor, usamos la lista exacta. Si no, usamos el ticker base.
    objetivos_exactos = traductor.get(tk, [])
    semilla_general = tk
    ...
    for nombre_mkt, url in mercados:
        try:
            ...
            for i in items:
                ...
                # REGLA DE FILTRADO QUIRÚRGICO
                es_match = False
                
                # A. Si es uno de nuestros objetivos exactos (Forex/Indices/Metales)
                if sym in objetivos_exactos:
                    es_match = True
                
                # B. Si es una cripto normal (Match casi exacto para evitar GASOLINE)
                # Solo aceptamos si el símbolo es "BTC-USDT" o "BTCUSDT"
                elif not objetivos_exactos:
                    if sym == f"{tk}-USDT" or sym == f"{tk}USDT" or sym == f"{tk}-USDC":
                        es_match = True
                ...
                if es_match:
                    ...
                    if precio:
                        encontrados.append({
                            ...
                            "Ticker": sym,
                            "Precio": precio,
                            "Info": "Verificado (Sintético/Spot)"
                        })
        except: continue
...
def ejecutor_maestro_v1_6():
    print(f"💎 CÓDIGO MAESTRO V1.78 - FILTRADO FINNHUB FX/CRYPTO")
