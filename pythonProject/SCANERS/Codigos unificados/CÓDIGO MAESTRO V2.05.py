...
# ==========================================================
# 💾 PERSISTENCIA EN sys_busqueda_resultados (V2.05 - SIN DUPLICADOS)
# ==========================================================
...
def guardar_en_resultados_db(conn, hallazgos, id_tarea, busqueda_original):
    try:
        ...
        for h in hallazgos:
            try:
                p_str = str(h.get('Precio', '0')).replace(',', '').strip()
...
# ==========================================================
# 🧹 FUNCIÓN DE MANTENIMIENTO (Limpieza cada 15 min)
# ==========================================================
def limpiar_resultados_antiguos(conn):
    """Borra registros de la tabla resultados que tengan más de 15 minutos"""
    try:
        cur = conn.cursor()
        # SQL para borrar resultados viejos basándose en la fecha de hallazgo
...
# ==========================================================
# 🚀 ORQUESTADOR V2.06 - CEREBRO CON MEMORIA INTELIGENTE
# ==========================================================

def bucle_operativo():
    print(f"💎 MOTOR MAESTRO V2.06 - ONLINE (Memoria Optimizada + Hostinger)")
    ...
    while True:
        try:
            ...
            if tarea:
                ...
                # Buscamos si existen resultados de CUALQUIER búsqueda anterior para este mismo ticker
                query_memoria = """SELECT motor, ticker, precio, info 
                                   FROM sys_busqueda_resultados 
                                   WHERE nombre_comun = %s 
                                   GROUP BY motor, ticker LIMIT 20"""
                cur.execute(query_memoria, (tk_busqueda,))
                existentes = cur.fetchall()

                if existentes:
                    print(f"🧠 MEMORIA: Datos encontrados para {tk_busqueda}. Sincronizando...")
                    ...
                    conn.commit() 
                    print(f"✅ Web Actualizada instantáneamente vía Memoria.")
                
                else:
                    # --- BÚSQUEDA FRESCA EN APIS ---
                    print(f"🔍 Interrogando mercados para {tk_busqueda}...")
                    consolidado = []
                    # Aquí asumo que tus funciones mapeo_... ya están definidas
                    motores = [
                    ...
                    for nombre, funcion in motores:
                        print(f"📡 {nombre: <12}", end=" ", flush=True)
                        try:
                    ...
                    if consolidado:
                        # Usamos tu función de persistencia original
                        guardar_en_resultados_db(conn, consolidado, id_tarea, tk_busqueda)
                        cur.execute("UPDATE sys_simbolos_buscados SET status = 'encontrado' WHERE id = %s", (id_tarea,))
                        print(f"✅ Sincronización exitosa con APIs.")
                    else:
                        cur.execute("UPDATE sys_simbolos_buscados SET status = 'error' WHERE id = %s", (id_tarea,))
                
                conn.commit()
                
                # --- TU LÓGICA DE TIEMPO (10 SEGUNDOS) ---
                print(f"⏳ Tarea finalizada. Pausa de 10s...")
                time.sleep(10)

            else:
                # --- MODO REPOSO (1 MINUTO) ---
            ...
            if err.errno == 1226:
                print("\n⚠️ Límite Hostinger (Max Queries) superado. Pausa de 5 min...")
                time.sleep(300)
            else:
                print(f"\n❌ Error DB: {err}")
                conn = None
