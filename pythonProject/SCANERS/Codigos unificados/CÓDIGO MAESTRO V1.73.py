...
def mapeo_bingx(busqueda):
    ...
    # Definimos los 3 mercados de BingX con sus URLs correctas
    mercados = [
    ...
    # Variantes para capturar sintéticos (GOLDX, AAPLX, etc.)
    variantes = [tk, f"{tk}X", f"NC{tk}", f"NCSK{tk}"]

    for nombre_mkt, url in mercados:
        try:
            r = requests.get(url, timeout=10).json()
            
            # 1. Normalizamos la extracción de datos según el mercado
            if nombre_mkt == "BINGX_SPOT":
                items = r.get('data', [])
            elif nombre_mkt == "BINGX_PERP":
                items = r.get('data', [])
            else: # Standard
                items = r.get('data', {}).get('tickers', [])

            # 2. Buscamos coincidencias
            for i in items:
                sym = i.get('symbol', '')
                
                # Si el ticker o sus variantes están en el símbolo (ej: GOLD en NCCOGOLD)
                if any(v in sym for v in variantes):
                    # Manejo de precios (algunas APIs usan 'lastPrice', otras 'price')
                    precio = i.get('lastPrice') or i.get('price') or i.get('last_price')
                    ...
                    if precio:
                        encontrados.append({
                            ...
                            "Ticker": sym,
                            "Precio": precio,
                            "Info": "Exchange Centralizado"
                        })
        except:
            # Si un mercado falla (como Standard), el código sigue con el siguiente
            continue
