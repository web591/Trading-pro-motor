...
# ==========================================================
# 💾 PERSISTENCIA ULTRA-LIMPIA (REUTILIZACIÓN DE FILAS)
# ==========================================================
def guardar_o_actualizar_resultados(resultados, nuevo_id_tarea, tk_comun):
    try:
        ...
        for res in resultados:
            ...
            # Si el Motor y el Ticker ya existen para este 'nombre_comun' (BTC), 
            # solo actualizamos el busqueda_id y el precio. NO INSERTAMOS NUEVO.
            query = """
            ...
            """
            cursor.execute(query, (nuevo_id_tarea, tk_comun, res['Motor'], res['Ticker'], res['Precio'], res['Info']))
        
        conn.commit()
        cursor.close()
        conn.close()
    except Exception as e:
        print(f"❌ Error al reutilizar datos: {e}")

# ==========================================================
# 🔄 BUCLE MAESTRO MODIFICADO
# ==========================================================
def ejecutar_bucle_buscador():
    print("💎 MAESTRO V3.01 - MODO REUTILIZACIÓN ACTIVO")
    ...
    while True:
        ...
        try:
            ...
            if tarea:
                ...
                print(f"\n🔍 SOLICITUD #{id_tarea} PARA: {tk_busqueda}")
                
                # 1. ¿Ya tenemos este ticker en la tabla (de cualquier búsqueda anterior)?
                cur.execute("SELECT motor, ticker, precio, info FROM sys_busqueda_resultados WHERE nombre_comun = %s", (tk_busqueda,))
                cache = cur.fetchall()

                if cache:
                    print(f"♻️ REUTILIZANDO: Pasando datos previos a la solicitud #{id_tarea}")
                    # En lugar de insertar nuevos, actualizamos los existentes para que el PHP los vea
                    ...
                    print(f"📡 API: Buscando por primera vez...")
                    consolidado = []
                    ...
                    guardar_o_actualizar_resultados(consolidado, id_tarea, tk_busqueda)

                # Vista en terminal...
                if consolidado:
                    print(pd.DataFrame(consolidado)[["Motor", "Ticker", "Precio"]].to_string(index=False))
