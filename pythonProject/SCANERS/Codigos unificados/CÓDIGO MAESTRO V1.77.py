...
def mapeo_bingx(busqueda):
    ...
    # 1. MAPEO DE ADN EXPANDIDO (Para no perder nada)
    mapeo_adn = {
    ...
    # Si el ticker está en el mapa, usamos esa familia. Si no, usamos el ticker base + variantes.
    familia = mapeo_adn.get(tk, [tk, f"{tk}X", f"NC{tk}", f"NCSK{tk}"])
    if tk not in familia: familia.append(tk)
    ...
    for nombre_mkt, url in mercados:
        try:
            ...
            for i in items:
                ...
                # LA REGLA DE SUMA: 
                # Si el símbolo de BingX contiene CUALQUIERA de nuestras palabras clave de la familia
                if any(miembro in sym for miembro in familia):
                    # Solo una pequeña validación para no traer basura extrema (como "GOLDEN_DOGE_INU")
                    # Si el símbolo es muy largo (>20 caracteres), probablemente es basura, si no, ADENTRO.
                    if len(sym) < 22:
                        ...
                        if precio:
                            encontrados.append({
                                ...
                                "Ticker": sym,
                                "Precio": precio,
                                "Info": "BingX Tradicional/Sintético"
                            })
        except: continue
...
def ejecutor_maestro_v1_6():
    print(f"💎 CÓDIGO MAESTRO V1.77 - FILTRADO FINNHUB FX/CRYPTO")
