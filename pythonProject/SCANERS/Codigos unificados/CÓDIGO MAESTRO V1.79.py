...
def mapeo_bingx(busqueda):
    ...
    # 1. TRADUCTOR MAESTRO (Aquí sumamos TODO lo que hemos ganado)
    # Si quieres que algo aparezca SIEMPRE, mételo en su lista aquí.
    mapeo_adn = {
    ...
    # Determinamos si estamos buscando una "Familia" o un Ticker suelto
    ...
    for nombre_mkt, url in mercados:
        try:
            ...
            for i in items:
                ...
                # Limpiamos el símbolo para la comparación (quitamos ruido de pares)
                sym_fix = sym.replace("-USDT", "").replace("USDT", "").replace("-", "")
                
                es_valido = False
                
                # REGLA A: Match directo con la familia (Cubre NCCOGOLD, XAUT, EUR, NVDAX)
                if any(miembro == sym_fix or miembro in sym for miembro in familia):
                    # FILTRO ANTI-BASURA: 
                    # Si es cripto (no está en nuestro ADN manual), el nombre debe ser corto.
                    # Si es de nuestra familia manual, confiamos ciegamente.
                    if tk in mapeo_adn:
                        es_valido = True
                    else:
                        # Si buscas SOL, sym_fix debe ser SOL (evita SOLTOMATO)
                        if sym_fix == tk:
                            es_valido = True
...
def ejecutor_maestro_v1_6():
    print(f"💎 CÓDIGO MAESTRO V1.79 - FILTRADO FINNHUB FX/CRYPTO")
