...
def mapeo_bingx(busqueda):
    tk = busqueda.upper()
    # Limpiamos la búsqueda por si el usuario pone EURUSD o EUR/USD
    tk_clean = tk.replace("/", "").replace("-", "").replace("=X", "")
    encontrados = []
    ...
    for nombre_mkt, url in mercados:
        try:
            ...
            for i in items:
                ...
                # 1. Normalizamos el símbolo de BingX para comparar
                # Quitamos lo que sobra: -USDT, USDT, USDC, y guiones
                sym_norm = sym.replace("-USDT", "").replace("USDT", "").replace("-USDC", "").replace("USDC", "").replace("-", "")

                es_match = False
                tipo = "Cripto"

                # --- LÓGICA DE ADN AUTOMÁTICA ---
                
                # A. MATCH EXACTO (Cripto y Stocks Directos)
                # Ej: SOL == SOL, AAPL == AAPL
                if sym_norm == tk_clean:
                    es_match = True
                
                # B. VARIANTES SINTÉTICAS (Sufijo X)
                # Ej: AAPLX, NVDAX, GERX
                elif sym_norm == f"{tk_clean}X":
                    es_match = True
                    tipo = "Sintético (X)"

                # C. ACTIVOS INSTITUCIONALES (Prefijos NC)
                # Buscamos si el ticker está contenido en un símbolo que empiece por NC
                # Ej: NCFXEURUSD o NCCOAAPL
                elif sym.startswith("NC") and tk_clean in sym:
                    # Validamos que no sea un falso positivo (ej: si buscas 'SOL' y sale 'GASOLINE')
                    # Si el ticker está justo después del prefijo (NCFX, NCCO, NCSK)
                    prefijos = ["NCFX", "NCCO", "NCSK", "NC"]
                    for p in prefijos:
                        if sym.startswith(p) and sym.replace(p, "").startswith(tk_clean):
                            es_match = True
                            tipo = "Forex/Stock Institucional"
                            break
...
def ejecutor_maestro_v1_6():
    print(f"💎 CÓDIGO MAESTRO V1.85 - FILTRADO FINNHUB FX/CRYPTO")
