...
def registrar_trade_completo(cursor, uid, t_data):
    try:
        ...
        cursor.execute(sql_global, (id_vinculo, uid, 'BINANCE', 'SPOT', 'TRADE', t_data['symbol'], t_data['quoteQty'], t_data['commission'], t_data['fecha_sql'], 'BINANCE'))
        
        sql_detalle = "INSERT IGNORE INTO detalle_trades (user_id, exchange, tipo_producto, exchange_fuente, tipo_mercado, id_externo_ref, fecha_utc, symbol, lado, precio_ejecucion, cantidad_ejecutada, broker, trade_id_externo) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        cursor.execute(sql_detalle, (uid, 'BINANCE', 'SPOT', 'BINANCE', 'SPOT', id_vinculo, t_data['fecha_sql'], t_data['symbol'], t_data['side'], t_data['price'], t_data['qty'], 'BINANCE', f"TRD-{id_vinculo}"))
        return True
    except: return False
    ...
    try:
        ...
        for item in diccionario:
            ...
            try:
                ...
                if raw_trades:
                    for t in raw_trades:
                        t_f = {'orderId': str(t['orderId']), 'symbol': t['symbol'], 'side': 'BUY' if t['isBuyer'] else 'SELL', 'price': float(t['price']), 'qty': float(t['qty']), 'quoteQty': float(t['quoteQty']), 'commission': float(t['commission']), 'fecha_sql': datetime.fromtimestamp(t['time']/1000).strftime('%Y-%m-%d %H:%M:%S')}
                        if registrar_trade_completo(cursor, uid, t_f): total_ingresados += 1
            except: continue
...
# ==========================================================
def run():
    print(f"💎 MOTOR v6.3.0 - MULTI-BROKER (PRODUCCIÓN)")
    while True:
