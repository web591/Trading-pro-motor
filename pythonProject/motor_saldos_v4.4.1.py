...
# ==========================================================
# Version 4.2.14
# BINANCE INCOME NORMALIZADO (AIRDROP + BONUS + REBATES)
# ==========================================================

def actualizar_income_extra_binance(client, conexion_db, user_id):
...
# 🆕 LIQUIDATION RECORD
# ==========================================================
def actualizar_liquidaciones_binance(client,conexion_db,user_id):
    ...
    try:
        ...
        for i in data:
            ...
            if i["incomeType"]!="LIQUIDATION":
                continue

            ref=f"BN-LIQ-{i['tranId']}"
...
def bingx_deposits(session, db, user_id):

    inicio = obtener_inicio_dinamico(db.cursor(), user_id)

    endpoint = "/openApi/wallets/v1/capital/deposit/hisrec"
    ...
    for d in data.get("data", []):
        ...
        VALUES (%s,%s,'BINGX','WALLET_FLOW','DEPOSIT',
        %s,%s,%s,'SPOT')
        ...
        (user_id,
         ...
         d['coin'],
         float(d['amount']),
         normalizar_fecha_mysql(d['createTime'])
        ))
...
# ==========================================================
# Version 4.2.12
# ANEXO CONTABLE — DEPOSITOS BINGX
# ==========================================================

def bingx_deposit_ledger(conn, user_id, api_key, secret):
...
# ==========================================================
# 🆕 BINGX DEPOSITS → LEDGER v4.2.12
# ==========================================================
def bingx_deposits_ledger(conexion_db,user_id,api_key,api_secret):
    ...
    try:
        ...
        r=bx_req(api_key,api_secret,
        "/openApi/wallets/v1/deposit/history",
        ...
        print(f"BingX DEP Error {user_id}: {e}")
...
def iniciar_motor():
    print(f"💎 GEMA v4.4.1 - RADAR + OPERATIVE PRUEBA 1")
    while True:
        try:
            ...
            for u in usuarios:
                ...
                if broker_actual == 'BINGX':
                    ...
                    bingx_deposits_ledger(db, u['user_id'], k, s)
