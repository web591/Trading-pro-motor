...
CICLOS_DEEP_AUDIT = 30  # Cada ~1 hora escanea Historial, Dust y Earn
...
def limpiar_ticker_profesional(symbol):
    """Normaliza el ticker para el radar (ej: BTCUSDT -> BTC)"""
...
def disparador_radar(cur, symbol):
    """
    AUDITORÍA: Usa sys_busqueda_resultados.
    Usa INSERT IGNORE para que la base de datos gestione la unicidad.
    """
...
# ==============================================================================
#   BLOQUE BINANCE (Completo: Saldos, Órdenes, Historial, Dust, Earn)
# ==============================================================================
def procesar_binance(key, sec, user_id, db, deep):
    try:
        ...
        # 2. Órdenes Abiertas -> Tabla sys_open_orders
        ...
        for row in cur.fetchall():
            sym = row['sym']
            try:
                for t in client.get_my_trades(symbol=sym, limit=500):
                    id_ext = f"BN-T-{t['id']}"
                    # Insertar en transacciones globales para contabilidad
                    cur.execute("INSERT IGNORE INTO transacciones_globales (id_externo, user_id, exchange, categoria, asset, monto_neto, fecha_utc) VALUES (%s,%s,'Binance','TRADE',%s,%s,FROM_UNIXTIME(%s/1000))", (id_ext, user_id, sym, (float(t['qty']) if t['isBuyer'] else -float(t['qty'])), t['time']))
            except: continue

        # 4. DEEP AUDIT: Recuperado de v2.1.6 (Deposits, Withdrawals, Dust, Earn)
        if deep:
            # Depósitos
...
# ==============================================================================
#   BLOQUE BINGX (Saldos Spot/Perp + Órdenes Spot)
# ==============================================================================
def procesar_bingx(key, sec, user_id, db, session):
    try:
        ...
        # 3. Órdenes Spot -> sys_open_orders_bingx
...
if __name__ == "__main__":
    ...
    print("🚀 MOTOR v2.2.8 - FULL AUDIT (DUST+EARN+DEPOSITS) & TABLAS CORRECTAS")
    while True:
        try:
            ...
            users = cur.fetchall()
            deep = (ciclo % CICLOS_DEEP_AUDIT == 0) # Gatillo para auditoría profunda
            for u in users:
