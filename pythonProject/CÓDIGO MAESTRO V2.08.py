...
def guardar_en_resultados_db(conn, hallazgos, id_tarea, busqueda_original):
    try:
        ...
        # 🚩 FILTRO 1: EXCLUSIÓN DE STABLES (Evita inundar con paja)
        stables_a_ignorar = ['USDT', 'USDC', 'DAI', 'BUSD', 'EUR', 'USD']
        if tk_clean in stables_a_ignorar:
            cur.execute("UPDATE sys_simbolos_buscados SET status = 'ignorado', info = 'Stablecoin excluida' WHERE id = %s", (id_tarea,))
            conn.commit()
            print(f"    🚫 EXCLUIDO: {tk_clean} es Stablecoin.")
            return

        # 🚩 FILTRO 2: LIMPIEZA DE "PAJA" (Basura visual como la del CSV)
        # Solo guardamos lo que no parezca ruido (menos de 12 caracteres y sin caracteres chinos)
        hallazgos_limpios = [h for h in hallazgos if len(h.get('Ticker', '')) <= 12]

        # 1. Inyectar en sys_busqueda_resultados (Para el cuadro amarillo)
        query_res = """INSERT INTO sys_busqueda_resultados 
                       (busqueda_id, nombre_comun, motor, ticker, precio, info) 
                       VALUES (%s, %s, %s, %s, %s, %s)"""
        
        for h in hallazgos_limpios:
            cur.execute(query_res, (
                id_tarea, tk_clean, h['Motor'], h['Ticker'], 
                float(str(h.get('Precio', '0')).replace(',', '')), 
                h.get('Info') if h.get('Info') else h['Ticker']
            ))

        # 2. ⚡ SIEMBRA EN TRADUCTOR (is_active = 0)
        # Tomamos el primero de la lista como sugerencia principal
        if hallazgos_limpios:
            best = hallazgos_limpios[0]
            m_fuente = best['Motor'].lower()
            
            # Categorización Automática
            if any(m in m_fuente for m in ['binance', 'bingx']):
                cat = 'LENDING' if tk_clean.startswith('LD') else 'SPOT'
            elif any(m in m_fuente for m in ['yahoo', 'finnhub', 'alpha']):
                cat = 'STOCK' if '.' in best['Ticker'] or len(tk_clean) > 3 else 'FOREX'
            else:
                cat = 'OTRO'
            ...
            sql_siembra = """
                ...
                VALUES (%s, %s, %s, %s, '0')
            """
            cur.execute(sql_siembra, (tk_clean.replace('LD',''), m_fuente, best['Ticker'], cat))
            
            # 3. 🎯 UPDATE CORREGIDO: Vincular ID y poner Status 'validar'
            # Recuperamos el ID del traductor (sea nuevo o ya existente)
            # Usar el ticker_motor exacto para encontrar el ID recién creado o existente
            cur.execute("SELECT id FROM sys_traductor_simbolos WHERE ticker_motor = %s AND motor_fuente = %s LIMIT 1", 
                       (best['Ticker'], m_fuente))
            res_trad = cur.fetchone()

            if res_trad:
                trad_id = res_trad['id'] # Si usas dictionary=True es ['id'], si no es [0]
                cur.execute("""
        ...
        conn.commit()
        print(f"    ✅ Tarea {id_tarea} procesada. Status: 'validar'.")

    except Exception as e:
        print(f"    ⚠️ Error en Guardado Consolidado: {e}")
...
def limpiar_resultados_antiguos(conn):
    ...
    try:
        ...
        if registros_borrados > 0:
            print(f"\n🧹 MANTENIMIENTO: Se eliminaron {registros_borrados} registros obsoletos.")
...
# ==========================================================
# 🚀 ORQUESTADOR V2.07 - CEREBRO CON AUTO-SIEMBRA
# ==========================================================
def bucle_operativo():
    print(f"💎 MOTOR MAESTRO V2.08 - ONLINE (Auto-Siembra + Hostinger)")
    print(f"📡 Escaneando tareas: Pendientes, Encontrados ")
    ...
    while True:
        try:
            ...
            # 2. BUSCAR TAREAS (Filtramos para ignorar los 'ignorado' y 'error')
            # Procesamos 'pendiente' (usuario) y 'encontrado' (radar de saldos)
            query_tarea = """
                SELECT id, ticker, status 
                FROM sys_simbolos_buscados 
                WHERE status IN ('pendiente', 'encontrado') 
                ORDER BY CASE WHEN status = 'pendiente' THEN 1 ELSE 2 END 
                LIMIT 1
            """
            cur.execute(query_tarea)
            tarea = cur.fetchone()

            # ... (dentro del bucle_operativo, después de cur.execute(query_tarea)) ...

            if tarea:
                id_tarea = tarea['id']
                tk_busqueda = tarea['ticker'].upper().strip()
                status_actual = tarea['status']
                
                # 1. BLOQUEO INMEDIATO
                cur.execute("UPDATE sys_simbolos_buscados SET status = 'procesando' WHERE id = %s", (id_tarea,))
                conn.commit()
                
                print(f"\n🎯 TAREA: {tk_busqueda} (ID: {id_tarea})")
                
                # --- PASO A: REVISAR MEMORIA ---
                query_memoria = "SELECT motor, ticker, precio, info FROM sys_busqueda_resultados WHERE nombre_comun = %s GROUP BY motor, ticker LIMIT 20"
                cur.execute(query_memoria, (tk_busqueda,))
                existentes = cur.fetchall()

                if existentes:
                    print(f"🧠 MEMORIA: {tk_busqueda} en caché...")
                    # Ejecutamos la siembra y el guardado
                    guardar_en_resultados_db(conn, existentes, id_tarea, tk_busqueda)
                    
                    # 🚩 CAMBIO CLAVE: Marcamos como 'terminado' o 'sincronizado' para que no lo vuelva a leer
                    cur.execute("UPDATE sys_simbolos_buscados SET status = 'terminado' WHERE id = %s", (id_tarea,))
                    conn.commit() 
                    print(f"✅ ID {id_tarea} finalizado por Memoria.")
                

                else:
                    # --- PASO B: BÚSQUEDA FRESCA ---
                    print(f"🔍 Buscando en APIs...")
                    consolidado = []
                    
                    # 💡 AQUÍ DEFINIMOS TODOS LOS MOTORES DISPONIBLES
                    motores = [
                    ...
                    # Ejecutamos cada motor uno por uno
                    for nombre, funcion in motores:
                        try:
                            ...
                            if res: 
                                ...
                                print(f"   ✅ {nombre} encontró {len(res)} resultados.")
                        except Exception as e:
                            print(f"   ❌ Error en motor {nombre}: {e}")
                            continue

                    if consolidado:
                        # Guardamos todo en la DB y sembramos en el traductor
                        guardar_en_resultados_db(conn, consolidado, id_tarea, tk_busqueda)
                    else:
                        # Si tras recorrer todos los motores no hay nada...
                        print(f"   ❌ No se hallaron resultados para {tk_busqueda}")
                        cur.execute("UPDATE sys_simbolos_buscados SET status = 'error' WHERE id = %s", (id_tarea,))

                conn.commit()
                print(f"⏳ Pausa de 10s para saltar al siguiente...")
                time.sleep(10)
                
            else:
                # MODO REPOSO: Si no hay nada, limpiamos y esperamos
