...
def bingx_income(db, user_id, key, secret):
    ...
    if r.get("code") == 0 and "data" in r:
        ...
        for i in r["data"]:
            ...
            # --- MAPEO DE CATEGORÍAS PARA DASHBOARD ---
            ...
            # Registro único que alimenta ambas tablas (Cashflow y Global)
            # Usamos los nombres de llaves que espera tu motor v1.2.7
            registrar_cashflow(cursor, {
                "user_id": user_id, 
                "broker": "BINGX", 
                "tipo_evento": categoria, # Aquí pasamos la categoría mapeada
                "asset": asset, 
                "cantidad": monto, 
                "ticker_motor": symbol,
                "fecha": fecha_sql, 
...
def bingx_deposits(db, user_id, key, secret):
    ...
    count = 0
    for d in r["data"]:
        fecha_sql = time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(d["insertTime"]/1000))
...
def ejecutar_motor_financiero(db):
    print(f"\n{'='*60}")
    print(f"💎 MOTOR FINANCIERO v1.2.8 - AUDITORÍA Y DIVIDENDOS")
    print(f"{'='*60}")
